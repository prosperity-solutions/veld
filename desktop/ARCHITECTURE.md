...
| Terminal session lifetime | Sessions outlive their socket; explicit `DELETE` ends one | A terminal is not re-creatable state — dropping it kills the shell and everything in it — and a reload drops the socket. So a socket closing means "come back later" (scrollback is buffered and replayed on reattach) while closing a *pane* means "done". The session budget is defended at the *creation* end rather than by disconnecting hidden panes: a worktree's default layout seeds a `new` pane instead of a terminal, so browsing the rail starts no shells at all (cap of 48, readable error on hitting it). Disconnecting a hidden pane's socket was the other candidate and is **rejected**, though the reason has moved: it used to be that a detached session is reapable, so a build in a worktree you switched away from would be hung up after the detach grace. It no longer is — a client declares every session its layouts name (`keep`, `crates/veld-daemon/src/ide.rs`) and the reaper skips those, so the grace measures the last *window* letting go rather than the socket. What still argues against disconnecting is what a reattach costs: a scrollback replay and a repaint of whatever foreground program is running, on every switch back. |
...
| Tab cycling across windows | **The main process owns the order** — every window pushes its strip (`veld:window:tabs`), `veld:window:cycle-tab` answers from all of them, and the window a tab lives in activates it itself (`veld:window:activate-tab`) | This was attempted once and pulled ([#315](https://github.com/prosperity-solutions/veld/pull/315), traced in [#316](https://github.com/prosperity-solutions/veld/issues/316)), with the ordering kept in the renderer: a per-window registry of the detached windows *that window had opened*. Every one of its four failure rounds traces to that choice. Such a registry cannot learn a window closed (so it pruned on a failed `focus` and retried), cannot see a window it did not open, and — the one that broke it — has nowhere to record "the cycle is parked on a window whose tabs I cannot read", which needed an anchor ref that went stale and ping-ponged between one tab and one detached window. Moving the order to the only process that can see every window deletes all three questions rather than answering them. **Position is derived, never stored**: it is `(the window the keystroke arrived in, the active tab it sent with the request)`, so after a press lands elsewhere that window has OS focus and the next press arrives from *it*, carrying its own truth. **And the activation happens in the target renderer**, through the same call a click makes (`PaneAreaHandle.selectTab`) — `show()`/`focus()` are OS-level and cannot reach into another renderer's DOM, which is what left #315 with a window that looked focused showing a tab that had not really reopened. The ordering arithmetic is pure and tested in `windowState.js` (`cycleOrder`, `nextInCycle`) rather than living inside an IPC handler, because it is precisely the region that kept breaking. |

**Traffic lights follow the page zoom.** The lights are OS-drawn at a fixed size while the top bar is CSS and scales with the page's zoom factor, so at any zoom but 100% the lights drift off the bar's own controls — the misalignment this feature exists to undo, in the manner Slack's does. The bar's centre is read at the current factor (`topbarHeight × zoom`) and the light is re-centred on it via `setWindowButtonPosition` (the runtime half of the `trafficLightPosition` set at creation). Detection is a **100ms poll of `getZoomFactor()`**, not an event, because Electron has no event covering every zoom source: `zoom-changed` fires only for the mouse wheel, while the View menu's `zoomIn`/`zoomOut`/`resetZoom` roles and their `⌘+`/`⌘−`/`⌘0` accelerators change the factor silently, and a window restored on a per-origin remembered zoom fires nothing at all. A poll that repositions only when the factor *moved* catches all of them for the cost of a cached read; a `did-finish-load` sync covers the restore-at-a-remembered-zoom case without waiting for a tick. Detached windows are exempt — they keep a normal frame, so there is nothing to reposition. The centring math is pure and lives in `windowState.js` (`trafficLightY`) where it has tests.

**The same zoom also keeps the *horizontal* gap fixed.** The lights are OS-drawn at a fixed size, so the top bar's traffic-light inset — the gap before the view switcher — must not scale with the page: a `100px` inset grows the gap on zoom-in and collapses it into the lights on zoom-out. The main process pushes the factor it already polls to the page (`veld:window:zoom`, the sync-read + subscription twin of the full-screen channel), `watchZoom` in `shell.ts` sets `--topbar-zoom` on the root, and the inset is `calc(100px / var(--topbar-zoom, 1))` — a fixed DIP width whatever the zoom.
...
| Browser pane right-click menu | **A native `Menu.popup()` raised from the view's own `context-menu` event**, with the item set built in a pure, tested `desktop/src/browserMenu.js` | Electron gives an embedded view no context menu at all, so the gesture did nothing — and *copy this link* / *copy that address* is most of what a browser's menu is for. Native rather than drawn in `/ide` because a DOM menu over a `WebContentsView` has to go through the freeze-and-hide dance in the *Native-view z-order* row: capture the page to a still, hide the live view, draw over the freeze, put the view back on dismiss. That is the right price for an address-bar dropdown you type into and the wrong one for a click-read-click gesture, where the page visibly freezing and thawing around the menu *is* the experience — and an OS menu also lands at the pointer with none of the CSS-pixel → DIP conversion every other geometry in `browserViews.js` needs. **The target wins and navigation is the fallback**, as in Chrome: a menu raised on a link carries no Back/Forward/Reload, because four items you did not aim at push the one you did further from the pointer. Two details are load-bearing. **No menu `role`s** — a role acts on whatever holds the keyboard and a right-click does not reliably move it, so clicking into a terminal and then right-clicking a link in the pane beside it would have copied the terminal's selection; every command is addressed to that pane's `webContents` by name. And **`wc.copy()` rather than `clipboard.writeText(params.selectionText)`**: Chromium truncates `selectionText`, so writing it copies a prefix of a long selection, and the editing command carries the HTML flavour a rich paste needs. Chromium raises the event only where it would have shown its own menu, so a page that `preventDefault()`s `contextmenu` keeps its own. Two things a review round added and both are load-bearing. **`mailtoAddress` rejects rather than repairs** — following `safeUserAgent`'s rule in `validate.js` — because `u.pathname` hands a `mailto:` over still percent-encoded and `decodeURIComponent` is what unwraps a pre-encoded `%0A` into a real newline, or `%E2%80%AE` into an address that renders backwards, on its way to the clipboard; rejecting is the *clean* answer and not merely the safe one, since `null` falls the menu back to *Copy Link Address*, where the smuggled codepoint stays visibly escaped and inert. Not printable-ASCII, though: an SMTPUTF8 address is legal, so what is refused is invisibility, not non-ASCII. And **`inspectElement` is preceded by an explicit `openDevTools({ mode: "detach" })`** — it does open DevTools on its own, but takes no mode, so it would use whatever dock state Chromium last saved and could dock an inspector inside the very view's box that the pane's ⟨⟩ button passes `detach` to avoid. `MENU_IDS` is exported from the pure module and `browserMenu.test.js` asserts both directions against `dispatchContextMenuAction`'s `switch` as text — sliced to that function first, because the unrelated `veld:browser:command` switch carries identical `case "back"/"forward"/"reload"` labels and a whole-file search false-*passed* for all three; a meta-test asserts the gate fails when a case is removed, since a gate nobody has seen fail is a gate nobody should believe. **The one piece of module state is the open menu itself** (`openMenus`, keyed per `(window, view)`): a menu outlives the click that opened it by however long the user reads it, so it is dismissed on re-popup and from `disposeEntry` — which makes it the third thing a closing pane can leave on screen, beside a detached inspector and an attached debugger, and it is closed in the same place for the same reason. `disposeWindow` runs from `win.on("close")` rather than `closed`, so the window is still alive when `closePopup` needs it. |
...
3. macOS tray (template icon): shows running-run count, per-run stop/restart
   ...
   tick that rebuilds the menu, and the renderer nudges it over
   `veld:app:settings-changed` so a toggle in the dialog is not a ten-second
   wait. The nudge carries **no value**: a page's copy of the document can be a
   stale `localStorage` mirror, and acting on that would put back an icon the
   daemon says is off — so the renderer says only *when*. That sync is
   **serialised** (`src/trayVisibility.js`), because the tick and a nudge
   overlapping across the read's own `await` both saw "no tray yet" and both
   created one. A failed read keeps the last answer: only an explicit `false`
   from a daemon that replied takes the icon away, so a daemon that is down or
...
4. `contextIsolation: true`, `nodeIntegration: false`, preload exposing
   ...
   renderer acts on its own window — with **four deliberate exceptions, all
   ...
   already gone); `veld:window:detach` opens a new window by definition;
   `veld:window:legacy-claim` raises whichever window currently holds a worktree
   the sender names — unless the sender passes `focusHolder: false`, which is how
   a bundle asks about several worktrees in a row without raising anything — for
   an `/ide` bundle older than this shell; and
   `veld:window:cycle-tab` may raise a *different* window and push
   `veld:window:activate-tab` at it, because the tab order it answers from spans
   every window showing the worktree. `senderFrame` still gates who may *ask*;
   the exception is only in which window the ask acts on.

   Two things keep that last one narrow, and it is worth being precise because an
   earlier draft of this paragraph overstated the first and omitted
   `legacy-claim` entirely. **The sender never names a window** — there is no
   window id in the request and none is handed back, so it can only step to
   whatever the order says comes next. But naming a *worktree* is most of the way
   to naming a window, so the handler also checks `ownsWorktree`: a window may
   only cycle a worktree it is itself showing, the same test drop routing and
   `legacy-claim` make.
...
| `POST /api/worktrees` `{repo_root, branch, alias?, display_name?, lane?, path?, source?, create_branch?, emoji?, marker_color?}` → `WorktreeView` + `carry_over?` | `git worktree add`. Default path (no explicit `path`): `<storage root>/<project slug>/<alias>`. The storage root is the configured directory when `worktree.storageMode`/`worktree.storageDir` say `"custom"` (`Db::worktree_storage_dir`), else `<repo_parent>/_worktrees` — today's only behaviour, and the fallback whenever `"custom"` is chosen but no folder has been picked yet. `project_slug` is `<slugified repo dir name>-<8 hex chars of a hash of the canonicalized repo root>`: the hash exists because `repos.name` (a bare basename) carries no uniqueness constraint, only `repos.root` does, so two differently-located repos sharing a directory name would otherwise be handed the same bucket — permanently blocking each other's common aliases (`main`, `feat`) with the `409` below, an outcome that can happen in `sibling` mode when they share a parent directory and is guaranteed in `custom` mode, which funnels every repo into one folder. Existing checkouts are never moved by changing the setting; it only decides where the *next* one lands. An explicit `alias` a sibling already holds is a `409`. The check runs *before* `git worktree add`, so the common case creates nothing; it is a plain read though, so a create that races another one (or a sibling on disk not yet synced) still 409s on the authoritative check after the checkout exists — what survives then is a registered worktree under its branch-derived alias, not an orphan. `lane` is pre-checked the same way and for the same reason (a `400` that created nothing beats a checkout filed in the wrong section), with `Db::patch_worktree`'s transaction still the authority. `display_name` is trimmed server-side and bounded by `validate_display_name`: ≤80 **characters** (not bytes), nothing that misrenders its neighbours (`is_forbidden` — control characters, U+2028/U+2029, bidi overrides and isolates, BOM), and at least one **visible** character. That last clause is the one a per-character blocklist cannot express and the one that matters: `""` means "render the alias", so a non-empty name of nothing but zero-width characters would render blank everywhere while defeating the sentinel. Zero-width joiners *beside* a visible character stay legal — they are the glue in emoji like `👩‍💻` and are orthographically required in Persian and Hindi. `emoji`/`marker_color` are the create dialog's marker pick, validated up front. The name and the marker are applied in one `patch_worktree` after the sync assigns its own, and *before* the alias rename, so a checkout that loses the alias race still wears them. **The lane is a deliberate second write**: it is the only one of the four validated inside `patch_worktree`'s transaction, so folding it in made that write fallible and a lane deleted mid-flight discarded the name and marker with it. Omit the marker and the daemon assigns. **`source` is a tagged enum** (`{kind}` + per-variant fields) naming where the checkout starts: `new_branch` (cut `branch` from `origin/<default>` or local HEAD per `git.createFrom` — the default, and what an absent `source` plus `create_branch: true` still means), `local_branch` (check out the existing `branch`; `create_branch: false`), `remote_branch` `{remote_ref}` (refuse unless `refs/remotes/<remote_ref>` exists — `git fetch <name>` treats an unconfigured name as a URL — then fetch that remote and `worktree add --track -b branch … remote_ref`), and `worktree` `{from_path, carry_over?}` (a spin-off: same-repo and non-trashed only, branch cut from that checkout's `HEAD` read **once** so the trees and the branch cannot describe different bases). **The source is a path, not a `worktrees.id`** — that column is a rowid with no `AUTOINCREMENT`, so SQLite reuses it, and the create dialog can sit open for minutes: a reused id would sail past both guards to branch off a checkout the user never picked. Same rule, same reason, as the two order endpoints below. Tagged rather than flat optionals so a remote ref on a local checkout, or a carry-over with nothing to carry from, is unrepresentable; an unknown `kind` is a `422` from axum's extractor rather than a silent fall-through to the default. `carry_over` captures the source's uncommitted state as **two tree objects** built from *copies* of its index (`GIT_INDEX_FILE` + `add -A` + `write-tree`), reconstituted with `read-tree -u --reset` then `read-tree` — which reproduces the staged/unstaged split exactly, carries deletions and untracked files, and excludes ignored ones because `add -A` does. It writes nothing of the source's but shared objects (`git stash create` is the tempting alternative and is disqualified: it rewrites the source's index under `index.lock` even with `--no-optional-locks`), and the scratch directory is `0700` outside the checkout, since after `add -A` that index holds the repo's whole path inventory. An unmerged index (`write-tree` exits 128, and the merge itself is per-worktree state no tree carries) and a sparse checkout (out-of-cone files read as deletions) are **refused before anything is created**. A failure *after* the checkout exists is reported in `carry_over: {files, drifted, error?}` rather than raised, because a 4xx would deny a worktree that is really there; `files` counts what arrived and can be non-zero beside an `error` (a partial apply), and `drifted` means the source's working tree changed between two stagings — measured by comparing the two trees, since porcelain status is byte-identical when an already-modified file's *contents* change. The count reuses the shared dirty-file **parser** against its own `status … -z -uall` invocation: the shared *helper* collapses an untracked directory to one record and honours `status.showUntrackedFiles=no` (which `-uall` overrides), while the parser is what knows a rename's origin field is not a second file — counting raw NUL fields instead got both of those wrong in turn. Every git call on this router now also strips `GIT_DIR`/`GIT_WORK_TREE`/`GIT_COMMON_DIR`/`GIT_INDEX_FILE` from the inherited environment, because `GIT_DIR` beats `-C` and the daemon can be auto-started from inside a hook or a `git bisect run`. |
...
| `GET /api/repos/branches?repo_root=` | The branches a worktree can be created from, for the create dialog's source picker: `{local: [{name, checked_out_in, upstream}], remote: [{name, local_name, has_local}]}` from two `for-each-ref` calls. `checked_out_in` is the load-bearing field — git allows one checkout per branch, so without it the picker offers choices that fail on click. Symbolic refs are skipped (`refs/remotes/origin/HEAD` shortens to the bare remote name, which is not a branch). Registered repos only, like every other repo-scoped endpoint, since it spawns git in a caller-supplied directory. **It does not fetch** — a GET on this router is side-effect-free by contract — so the remote half is as fresh as the last fetch, and only `origin` is kept fresh by `refresh_repos`' throttled `maybe_fetch`; the create path fetches the chosen ref's own remote, so a stale list never produces a stale checkout. Uncapped, with the pickers capping what they *render* instead, so search still reaches every branch. |
...
| `GET /api/lanes?repo_root=` / `POST /api/lanes` `{repo_root, name}` | Rail lanes: user-named groups of worktrees, per repo. Names are trimmed, ≤32 chars, reject control characters, and are unique per repo **case-insensitively** (`409`). Max 32 per repo. Lanes also ride along on `GET /api/repos` so the rail never renders a worktree whose lane it has not heard of. **One extra row may be present and is not a lane:** the reserved, NUL-prefixed `UNGROUPED_LANE`, which records where the ungrouped "Worktrees" section sits in the rail order. It is exempt from the 32 cap, cannot be created, renamed, deleted or filed into (all four are refused), and a client that lists groups must filter it out — the UI's `realLanes` does. It is absent until the user first reorders; absence means "Worktrees renders first". |
...
| `POST /api/lane-order` `{repo_root, order}` | Lane display order, by name. The list may include the reserved `UNGROUPED_LANE`, which is how the ungrouped section's place is set; this is the one name the endpoint will **insert** rather than only update, and the one whose omission leaves its row alone instead of sweeping it to the tail with the other unmentioned lanes (a client that omits it has not heard of it, and must not silently relocate it for everyone else). Deliberately **not** `/api/lanes/order`: a static segment beats a dynamic one, so that path would shadow `/api/lanes/{name}` and make a lane called "order" impossible to rename or delete. |
...
| `POST /api/pick-directory?purpose=` | Opens the native OS folder picker (the daemon runs in the user's GUI session — macOS `osascript`, Linux `zenity`/`kdialog`) and returns `{path}`; `204` on cancel, `409` while a picker is already open (single-flight), `408` after the 10-minute timeout, `500` on backend failure (no GUI session / permission denial), `501` without a picker backend. Works for the plain-browser build too — the web platform never exposes absolute paths. Reused by the settings dialog's worktree-storage-location Browse… button, the same as the "Import repository" dialog — `purpose` (`pick_directory_prompt`) picks the dialog's prompt from a small fixed server-side set matched against the query value, never interpolated from it, so the folder picker asks for the right thing without becoming a second place to inject text into an AppleScript string. |
| `POST /api/open-worktree-storage-dir` | Opens the *effective* worktree storage directory (`Db::worktree_storage_dir`) in the OS file manager — macOS `open`, Linux `xdg-open`, detached rather than awaited to completion (a short grace window still catches an immediate exec failure). Takes no path from the caller. `204` on success, `404` when the configured directory doesn't exist or isn't a directory, `409` in the default "next to each repository" mode (there is no single directory to open there, only each repo's own sibling folder), `500` if the file manager itself fails to launch. |
