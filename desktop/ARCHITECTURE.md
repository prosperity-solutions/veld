...
makes reused rowids harmless), plus `trashed_at` and `trash_error` (background
removal — the row is both the record of intent and the work queue).
...
| `DELETE /api/worktrees/{id}?force=` | **Records intent and returns `202`** — it no longer waits for git. `worktrees.trashed_at` is set and a daemon worker (`worktree_trash.rs`) stops any run in the worktree, runs an un-forced `git worktree remove`, and drops the row; a failure clears `trashed_at` and records `trash_error`, putting the worktree back in the rail with the reason. With `?force=true` the removal runs **inline** (`204`/`422`), because the user is answering a refusal they have already been shown — and it refuses with `409` if a run is still live, since the forced path does not stop runs. Never deletes the main checkout (`Db::trash_worktree` enforces that, so eviction cannot bypass it). Prunes git bookkeeping if the checkout was already gone. |
| `POST /api/worktrees/{id}/restore` | Clears `trashed_at` — "keep it after all". `404` if the worker already removed it; the worker re-reads the flag immediately before the destructive step, so a restore that returns `200` is honoured. |
| `DELETE /api/worktrees/{id}/trash-error` | Clears a recorded removal failure (the user has read it). |
