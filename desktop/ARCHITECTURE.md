...
**Only a window the shell *reopened* reads the durable **slot** store** (`restored` on the bridge, checked in `readLayouts`). Slots are recycled lowest-first and a slot's key is never cleared, so to a `⌘N` window the layout sitting there is a dead one that happens to share a number — restoring it meant attaching to terminal ids another window was using, and an attach takes a shell over. This governs *detached* windows, which are the ones still keyed by slot; a main window reads the shared per-worktree store instead, where picking a worktree up is the point rather than a collision (see the Worktree ownership row).
...
| Worktree ownership | **One set of panes per worktree, one window showing it.** Layouts live in a store keyed by worktree and shared by every window; the shell records which window displays which (`claims` in `windows.js`) | Keying the durable store per *window* was the first design and it was wrong in the way that matters: opening one worktree in two windows grew two independent sets of terminals and browser panes, closing either stranded its set until the detach grace killed it, and nobody could hold in their head which window had which shells. It prevented the collision by creating the confusion. Now the storage is shared and an explicit claim keeps two windows off one set — a better fit than the slot ever was, because it can say *why*: picking a worktree another window has focuses that window instead of silently making a second copy. A window keeps the panes of worktrees it has visited (so switching back is instant rather than a reconnect and a scrollback replay) and gives one up only when another window claims it, which the shell asks for by name — **and waits to be told it happened.** The claim is recorded synchronously, so a third window asking during the wait is refused rather than granted alongside; the *answer* to the claimer is held until every other holder has acknowledged the yield, because it attaches to the worktree's terminals on the strength of that answer and a second attach takes a session over rather than mirroring it. Both windows rendering one worktree's panes for the length of a fire-and-forget `send` was a real window, not a theoretical one, and it ends with the loser's terminal dead-but-visible — the kind of thing that shows up on a loaded machine and nowhere else. A holder that cannot answer is given `YIELD_ACK_MS` and then proceeded past (with a warning, since that is the one remaining path that can reinstate the race), because a claim blocked forever is worse than the race it was avoiding; a holder that *closes* mid-yield is settled immediately rather than waited out. **Only a holder that has reported an acknowledging listener is waited for at all**, and it is reported by the effect that sends the acknowledgement rather than inferred from a signal that merely correlates — so deleting that effect withdraws the promise with it, and the shell degrades to the old fire-and-forget send instead of waiting out a timeout it can never satisfy. That also covers the skew case: an older `/ide` bundle has no such channel, and waiting on it would put the full timeout in front of *every* handover on a daemon-older-than-shell install. The acknowledgement is sent from a passive effect rather than from the IPC listener, because the release runs inside a `setLayouts` updater — acknowledging on receipt would promise a release React had not performed yet. Releasing on every switch was tried and made the common case — one window, switching back and forth — pay for a collision that cannot happen there. `releaseTerminal`, never `disposeTerminal`: the shells keep running and the layout stays in the shared store, which is what makes "close that window and it comes back over here" need no hand-off protocol at all. Detached windows are exempt on both counts — their tabs were transferred out of a worktree a main window owns, so they keep the per-slot store and never claim. **Ownership is visible before it is enforced**: the shell pushes each main window the worktrees the *others* are showing, and the rail marks those rows. Without it the model reads as a bug — the row does not open, a different window jumps forward, and nothing on screen connects the two. Dimmed with an icon rather than disabled, because the row still does something (it takes you there), and a toast in the window you clicked from says so, which is what you find when you come back. Pushed on every claim, close and load rather than polled, since every transition that matters happens in some other window. |
...
- **A shell outliving the daemon is only useful if the tabs naming it do too.**
  ...
  session ids survived as well. In Veld Desktop the layout is therefore written
  to a slot-keyed durable store beside `sessionStorage` (`layoutSlotKey` in
  `panes/model.ts`, `layoutSlot` in `shell.ts`), so quitting the app and
  reopening it reattaches instead of orphaning. Two windows must never restore
  one layout: an attach *takes over*, so they would ping-pong every shell between
  them — which is why the slot comes from the shell instead of being assumed.
