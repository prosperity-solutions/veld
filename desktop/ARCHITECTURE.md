...
1. Own the app's **windows** (`src/windows.js`, with the pure half — slot naming,
   ...
   for a fresh detach, `--veld-window-seed`; the window set is persisted to
...
4. `contextIsolation: true`, `nodeIntegration: false`, preload exposing
   ...
   the main frame, so a renderer can only ever act on its own window and an
   iframe inside a pane can act on none.
