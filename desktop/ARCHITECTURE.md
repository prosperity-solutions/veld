...
| `POST /api/pick-directory` | Opens the native OS folder picker (the daemon runs in the user's GUI session — macOS `osascript`, Linux `zenity`/`kdialog`) and returns `{path}`; `204` on cancel, `409` while a picker is already open (single-flight), `408` after the 10-minute timeout, `500` on backend failure (no GUI session / permission denial), `501` without a picker backend. Works for the plain-browser build too — the web platform never exposes absolute paths. Reused by the settings dialog's worktree-storage-location Browse… button, the same as the "Import repository" dialog. |
| `POST /api/open-worktree-storage-dir` | Opens the *effective* worktree storage directory (`Db::worktree_storage_dir`) in the OS file manager — macOS `open`, Linux `xdg-open`. Takes no path from the caller. `409` in the default "next to each repository" mode: there is no single directory to open there, only each repo's own sibling folder. |
