...
|-------|------------------|-------|
| Icon logo (`V.`) | `logo.svg` (repo root) | White `V` + accent-green dot, 32×32 viewBox. `crates/veld-daemon/ui/src/components/LogoMark.tsx` is the same geometry with the fills swapped for `var(--text)` / `var(--accent)`, because `/ide` renders in both palettes and the source file's baked-in white is for rasterising |
| Wordmark (`veld.`) | `crates/veld-daemon/assets/management-ui.html` (header SVG) and `crates/veld-gateway/src/pages.rs` (`WORDMARK_SVG`) | Letters take `var(--text)`, the final dot path takes `var(--accent)`. The default mark for a user-facing surface. Deliberately **not** in the `/ide` top bar — that bar is 40–42px and its first slot is the Runs↔IDE switcher, so it takes the icon mark under the checklist's tight-chrome exception below |
...
- [ ] Wordmark embedded (SVG, CSS-colored), accent dot green. The **icon mark**
      ...
      every slot is a control — today that is exactly the `/ide` top bar
      (`crates/veld-daemon/ui/src/components/LogoMark.tsx`). A page, panel,
