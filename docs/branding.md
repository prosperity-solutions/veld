...
| Veld Desktop menu-bar icon | `desktop/assets/trayTemplate.png` (+`@2x`) | The `logo.svg` mark as a macOS **template** image (black + alpha), so the OS tints it for the current menu bar. Same mark as the Hammerspoon widget's `integrations/hammerspoon/Veld.spoon/icon.png`, so veld has one menu-bar identity. Generated by the same script. Black + alpha only — a template image's colour is discarded |
