...
| Favicon | `website/favicon.svg` | Rounded dark square (`#0A0A0B`), white `V`, accent dot — the same mark the Desktop app icon rasterises. Inlined as a data-URI on every surface below; edit the canonical file and run `just favicon` to see who lags |
...
from `website/favicon.svg` and greps for it — run by `just lint` and by CI's
`schema` job. Adding a sixth surface means adding it to that script's list.
The one deliberate omission is the feedback overlay: it is injected into someone
else's page and must never replace that page's icon.
