...
|-------|------------------|-------|
| Icon logo (`V.`) | `logo.svg` (repo root), plus `crates/veld-daemon/ui/src/promotions/Brand.tsx` (`Logomark`, the React copy for `/ide`) | White `V` + accent-green dot, 32×32 viewBox. Not in the `/ide` **top bar**: that bar is 40–42px of controls and the Runs↔IDE switcher (which holds its only first-class slot) carries a swap glyph instead of a brand mark — see the checklist's tight-chrome exception below. It *is* used **inline in body text**, where the wordmark is too wide to sit in a line: the what's-new panel's per-card byline ("`V.` – Official veld news") is one 10.5px line, and four letterforms at that height compete with the sentence above them. The React copy drops the asset's hard-coded fills — the `V` takes `currentColor` so it dims with the byline, the dot keeps `var(--accent)` — and the dot is already the source's `:last-child`. The favicon and the Desktop app/tray icons keep the geometry |
| Wordmark (`veld.`) | `crates/veld-daemon/assets/management-ui.html` (header SVG), `crates/veld-gateway/src/pages.rs` (`WORDMARK_SVG`), and `crates/veld-daemon/ui/src/promotions/Brand.tsx` (`Wordmark`, the React copy for `/ide`) | Letters take `var(--text)`, the final dot path takes `var(--accent)`. The default mark for a user-facing surface. Deliberately **not** in the `/ide` *top bar* — that bar is 40–42px of controls, and its Runs↔IDE switcher carries a swap glyph rather than any brand mark (see the checklist's tight-chrome note). `/ide`'s **pages and dialogs** are a different matter and do take it: the first-run start screen carries it, which is the checklist's ordinary rule, not an exception. The what's-new panel is the one dialog that does not, and deliberately — it mixes Veld's cards with a project's, so a wordmark over the whole panel would be Veld putting its name above text a teammate wrote; provenance is per-card there, and each card carries the icon mark or the project's name instead. The React copy reorders the source SVG's paths so the dot is `:last-child`, matching the CSS convention below |
| Favicon | `website/favicon.svg` | The `logo.svg` mark translated **+8/+8** onto a rounded `#0A0A0B` tile in a 48-box — i.e. the same geometry `make-icons.py` rasterises for the app icon, so the dot keeps hanging one unit below the `V`'s foot. Inlined as a data-URI on every surface below; edit the canonical file and run `just favicon` to see who lags |
| Veld Desktop app icon | `desktop/assets/icon.png` | 1024², the favicon's shape rasterised. Generated by `desktop/scripts/make-icons.py` — don't hand-edit. Inset in a transparent margin, which is what macOS draws its shadow into |
| Veld Desktop menu-bar icon | `desktop/assets/trayTemplate.png` (+`@2x`) | The `logo.svg` mark as a macOS **template** image (black + alpha), so the OS tints it for the current menu bar. Generated by the same script. Black + alpha only — a template image's colour is discarded |

**The favicon is one mark in five files, tied together by a gate.** A page that
serves it must stay self-contained, so the mark cannot be an `<img src>` — it is
...
What the gate cannot see is the geometry: that `website/favicon.svg` is
`logo.svg`'s two paths translated +8/+8, and that
`desktop/scripts/make-icons.py` holds a third copy of the same vertices as
Python tuples. Checking that would mean parsing path data, so it stays prose —
if you move the mark, move all three, and note that the tile is the favicon's
alone (the app icon re-centres on `MARK_BOX` and insets for macOS's shadow).

**Changing the accent is a three-file edit.** It is a CSS token in the product UI
(`crates/veld-daemon/ui/src/styles.css`), a literal in `website/index.html`, and a
byte triple in `desktop/scripts/make-icons.py` — a PNG cannot reference a CSS
variable, so the generated icons carry their own copy. Grep for `C4F56A` and you
find all three.

A menu-bar icon is the one place the accent dot cannot be green: a template image
carries alpha only, and the alternative — shipping the coloured mark — is a white
glyph that disappears against a light menu bar. The dot stays as a *shape* there,
and the app icon beside it carries the full colour.
...
- [ ] Wordmark embedded (SVG, CSS-colored), accent dot green. The **icon mark**
      substitutes for it only where the surface is chrome under ~48px tall whose
      every slot is a control. Today no surface claims the exception: the `/ide`
      top bar's Runs↔IDE switcher shows a swap glyph rather than the mark, so in
      that bar the app's brand rides on the favicon, the window/app icon and the
      daemon's own pages. A page, panel,
      dialog or header has room for the wordmark and takes it; name the new
      surface here if you claim the exception, so "too tight" stays a list rather
      than an argument. `/ide`'s own full-page and dialog surfaces follow the
      ordinary rule — the first-run start screen and the What's-new panel each
      embed `Wordmark` from `promotions/Brand.tsx`
...
- [ ] Data-URI favicon — the canonical mark, copied verbatim from
      `website/favicon.svg`, and the new file added to `tests/validate-favicon.sh`
