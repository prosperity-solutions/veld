...
| Wordmark (`veld.`) | `crates/veld-daemon/assets/management-ui.html` (header SVG) and `crates/veld-gateway/src/pages.rs` (`WORDMARK_SVG`) | Letters take `var(--text)`, the final dot path takes `var(--accent)`. The default mark for a user-facing surface. Deliberately **not** in the `/ide` top bar — that bar is 40–42px of controls, and its Runs↔IDE switcher carries a swap glyph rather than any brand mark (see the checklist's tight-chrome note) |
...
- [ ] Wordmark embedded (SVG, CSS-colored), accent dot green. The **icon mark**
      ...
      top bar's Runs↔IDE switcher shows a swap glyph rather than the mark, so
      the app's brand rides on the favicon, the window/app icon and the daemon's
      own pages. A page, panel,
