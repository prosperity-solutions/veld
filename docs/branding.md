...
| Wordmark (`veld.`) | `crates/veld-daemon/assets/management-ui.html` (header SVG) and `crates/veld-gateway/src/pages.rs` (`WORDMARK_SVG`) | Letters take `var(--text)`, the final dot path takes `var(--accent)`. Deliberately **not** in the `/ide` top bar — that bar is dense and its first slot is the Runs↔IDE switcher, which carries the *icon* mark instead. A surface may satisfy the checklist below with either mark |
...
- [ ] Wordmark — or, where the chrome is too tight for it, the icon mark —
      embedded (SVG, CSS-colored), accent dot green
