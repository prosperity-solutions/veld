...
| Veld Desktop menu-bar icon | `desktop/assets/trayTemplate.png` (+`@2x`) | The `logo.svg` mark as a macOS **template** image (black + alpha), so the OS tints it for the current menu bar. The same mark the Hammerspoon widget shows (`integrations/hammerspoon/Veld.spoon/icon.png`), rendered separately — that asset predates this generator and is **not** regenerated by it, so the two will drift on the next brand change. It is also non-template (`init.lua` passes `false`), which is why it is a white glyph on a light menu bar; fixing that belongs to the widget. Generated by the same script. Black + alpha only — a template image's colour is discarded |
