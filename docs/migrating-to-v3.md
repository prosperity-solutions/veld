...
- **`hooks` and `ide`.** Opaque to veld. A `command` key *inside* them is not
  ...
  there. (This is the rule the removed converter broke.) Three keys under `ide`
  are now interpreted — `quicklinks`, `permissions` and `panes`, see
  [configuration.md](configuration.md#ide-quicklinks-permissions-and-panes) — but
  they are additions rather than a migration: a config that has none of them is
  unaffected, and the rest of `ide` stays opaque, including the `command`
  exemption. Note that `ide.panes[]` entries *do* use `argv`/`shell` and are
  read by veld, so unlike the rest of `ide` they follow the v3 command rules.
