...
# ---------------------------------------------------------------------------
# Test 5: Manual injection mode (inject: false)
# ---------------------------------------------------------------------------
echo ""
echo "--- Manual Injection Mode ---"

# Stop the running instance.
"$VELD_BIN" stop --name "$RUN_NAME" 2>/dev/null || true
sleep 1

# Create a temporary veld.json with inject: false.
MANUAL_DIR=$(mktemp -d)
cp -r "$PROJECT_DIR"/* "$MANUAL_DIR/" 2>/dev/null || true
cat > "$MANUAL_DIR/veld.json" <<'EOJSON'
{
  "$schema": "../../schema/v1/veld.schema.json",
  "schemaVersion": "1",
  "name": "testproject-nextjs",
  "url_template": "{service}.{run}.nextjs.localhost",
  "features": {
    "inject": false
  },
  "nodes": {
    "web": {
      ...
      "variants": {
        "local": {
          "type": "start_server",
          "command": "npx next dev --turbopack --port ${veld.port}",
          "health_check": { "type": "http", "path": "/", "expect_status": 200, "timeout_seconds": 60 }
        }
      }
    }
  }
...
MANUAL_RUN="manual-test-$$"
cd "$MANUAL_DIR"
"$VELD_BIN" start web:local --name "$MANUAL_RUN"

MANUAL_URLS=$("$VELD_BIN" urls --name "$MANUAL_RUN" --json 2>/dev/null)
MANUAL_URL=$(echo "$MANUAL_URLS" | grep -o '"https://[^"]*"' | head -1 | tr -d '"')

if [[ -z "$MANUAL_URL" ]]; then
  echo -e "  ${YELLOW}⊘${NC} Manual injection tests skipped (could not determine URL)"
else
  # Wait for readiness.
  for i in $(seq 1 30); do
    if curl -sk -o /dev/null -w '' "$MANUAL_URL/" 2>/dev/null; then break; fi
    sleep 2
  done

  MANUAL_HTML=$(curl -sk "$MANUAL_URL/" 2>/dev/null)

  assert_not_contains \
    "No auto-injection when inject: false" \
    "$MANUAL_HTML" \
    "__veld_cl"

  assert_http_ok \
    "/__veld__/feedback/script.js still reachable" \
    "$MANUAL_URL/__veld__/feedback/script.js"

  assert_http_ok \
    "/__veld__/api/client-log.js still reachable" \
    "$MANUAL_URL/__veld__/api/client-log.js"
fi

# Clean up manual test.
cd "$PROJECT_DIR" 2>/dev/null || true
"$VELD_BIN" stop --name "$MANUAL_RUN" 2>/dev/null || true
rm -rf "$MANUAL_DIR"
