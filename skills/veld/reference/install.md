...
This installs the `veld` binary, `veld-daemon`, `veld-helper`, and Caddy to `~/.local/bin` and `~/.local/lib/veld/`. In **privileged** mode the helper is served from a root-owned directory instead (`/var/db/veld-helper` on macOS, `/var/lib/veld-helper` on Linux), because it runs as root and a binary the installing user can overwrite is root at the next reboot. Updates still need no sudo: the installer hands the new binary to the running root helper, which verifies its signature and refuses anything older than itself.
