...
triggers:
  ...
  - feedback loop
  - "*.localhost"
compatibility: Requires veld v10.13.0+
...
- **`unshare`/`leave` ids are optional** — omit the id to resolve the sole active share/join; `veld stop` **and `veld restart`** auto-unshare the run's shares (a restart mints a new run id, so the old share would point at torn-down ports) and a consumer's join self-tears-down when the tunnel closes. Re-share after a restart if you still need the URL. Not yet covered: `veld start` over a *live* environment of the same name replaces the run without releasing its shares (issue #171) — `veld unshare` it manually
