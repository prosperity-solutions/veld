...
triggers:
  ...
  - feedback loop
  - "*.localhost"
compatibility: Requires veld v15.0.0+
...
- **`hooks` is reserved**, and so is every key under **`ide`** except `quicklinks` and `permissions` — the reserved ones parse and are stored but are **not executed or rendered** by this version; `veld lint` emits a notice naming them. (`ide` was spelled `ui` until it gained a meaning; the old spelling now errors as an unknown top-level key, and the message names the rename.)
