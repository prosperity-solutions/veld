...
triggers:
  ...
  - feedback loop
  - "*.localhost"
compatibility: Requires veld v16.33.0+
...
- **`ide.extensions` puts the project's own badges, buttons and menus in the top bar** — one flat array, `slot` is a *field* (`topBar` today) and `type` discriminates (`status` \| `action` \| `menu`). A `status` runs a command in the worktree and renders its stdout as `{ text, tone, tooltip, href, open_in, actions }`; **non-contract output becomes the text** (so `git rev-parse --short HEAD` is a working badge), **exit 0 with no output hides the badge**, and **a non-zero exit renders it red with the last stderr line on hover**. A badge's `actions` are **ids of declared `action` entries, never commands** — veld resolves each against the on-disk config, so a running command chooses among declared commands and cannot introduce one. **Omit `slot` on an `action`** to declare one reachable only from a `menu`'s `items` or a badge's `actions`, which is how a project offers three editors without three buttons. The output may also carry an **`icon`** (same allowlist/emoji as the declaration's, and it **overrides** it — a badge can change its glyph with its state). **Right-click a badge to re-run it**, or all of them; a forced refresh ignores `refresh_seconds` (3s floor instead) and reports its own errors, unlike the silent background poll, and running an `action` forces a re-read since an action usually changes what a badge says. Variables are the pane set minus `pane.*` (`${veld.root}`, `${veld.branch}`, `${veld.worktree}`, `${veld.project}`, `${veld.username}`); anything else is a lint problem, not a runtime failure. **Choosing `open_in` is the field to get right when setting this up for a project: anything behind a login the developer holds — code host, CI, cloud console, error tracker — is `system`, because a pane has its own cookie jar and lands them on a sign-in page (an SSO flow in a fresh partition is a dead end); `pane` is for what the run itself serves (localhost, staging on the same session, a local report).** Relatedly, **never put `requires_bin` on something with a GUI**: `code`/`webstorm`/`idea` are launchers installed *separately* from the editor, so a PATH check hides the option on a machine that has the app — leave the predicate off and let the command fall back to the bundle (`command -v code >/dev/null 2>&1 && exec code "${veld.root}" || exec open -a "Visual Studio Code" "${veld.root}"`). `open_in` defaults to **`system`** — the opposite of `ide.quicklinks` — because a badge's link is a provider page the user is already signed in to. `when_missing` (`hint` default, `disable`, `hide`) decides what a missing `requires_bin` looks like, and an explicit value **beats** the user's *hide disabled actions* setting, so an install hint cannot be silenced by a clutter preference. Bounds veld owns, not the repo: only the visible worktree is evaluated and only while a window is open, several windows share one child process, a request inside `refresh_seconds` is answered from the previous run, no tty and closed stdin (an unauthenticated CLI fails with its login hint instead of hanging), a 20s deadline killed by process group, output capped, `refresh_seconds` floored at 15, **24** extensions max, and every argv logged. `extensions.autoRefresh` (user setting, default on) turns the unattended half off machine-wide while leaving clicks working. **No consent prompt, deliberately** — the rationale is in `docs/extensions-vision.md`. Full field table: `reference/config.md`
