...
triggers:
  ...
  - feedback loop
  - "*.localhost"
compatibility: Requires veld v16.67.0+
...
- **`ide.quicklinks` URLs interpolate the worktree scope**, so a bookmark can open *this* checkout's branch: `{"label":"This branch on GitHub","url":"https://github.com/o/r/tree/${veld.branch_raw}"}`. Available names are the same closed set an `ide.extensions` command gets — `${veld.root}`, `${veld.branch}`, `${veld.branch_raw}`, `${veld.worktree}`, `${veld.project}`, `${veld.username}` — and a run's own values (`${output.*}`, `${nodes.*}`) are a `veld lint` finding, because a bookmark list renders whether or not anything is running. Three gotchas: use **`branch_raw`**, since `branch` is slugified and a `feat/foo` checkout would address a branch named `feat-foo`; values are **percent-encoded with `/` preserved** (a branch's slashes are path, but a `#` git allows in a branch name is escaped so it cannot truncate the URL at a fragment); and the **provider path is yours to write** (`/tree/` GitHub, `/-/tree/` GitLab) because veld never learns your code host. A reference that cannot resolve for a checkout omits the link for that checkout rather than shipping a half-substituted URL
