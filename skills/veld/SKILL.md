...
- **A `secret` value must not be *interpolated* into `argv`/`shell`** — a command line lands in the process table. Deliver it via `env` or `files`. A bare `$SECRET_NAME` inside a `shell` string or an `sh -c` script is fine and is the recommended form (the shell expands it at runtime, in the child, so it never enters argv); so is `["docker","run","-e","NAME","img"]`. Refused: `${vars.x}` / `${output.x}` / `${nodes.a.x}` anywhere in a command, `${NAME}` (veld resolves every `${…}` itself), and a bare `$NAME` in an argv element that is not a shell script
...
- **The root config may be `veld.json` or `veld.jsonc`** — both are read identically; both in one directory is an error. `veld init` writes `veld.json`. Included files were always glob-matched, so `nodes/*.jsonc` already worked
