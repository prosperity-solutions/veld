...
pub fn pi_extension_doc(cli: &Path, session_id: &str) -> String {
    ...
    format!(
        r#"// Generated by veld — rewritten on every launch, never edit by hand.
...
pub fn settings_path(shim_dir: &Path, tool: AgentTool, session_id: &str) -> PathBuf {
    ...
    shim_dir
        .join("agent")
        .join(format!("{}-{}.{ext}", tool.as_str(), sanitize(session_id)))
}
...
mod tests {
    ...
    fn a_settings_path_is_per_session_and_never_escapes_its_directory() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/tmp/shims/agent/pi-abc-1.ts")
        );
    }
