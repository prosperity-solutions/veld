          ...
          >
            <Stack gap="md">
              ...
              <Row
                ...
                help="Works with Claude Code and Codex CLI. Neither tool's output says whether it is thinking or waiting — measured against Claude Code, which emits a title sequence, hyperlinks and a progress report, and nothing about its state. So Veld puts a wrapper on the terminal's PATH that hands the real binary an extra, throwaway hook configuration: a settings file for Claude, a `-c notify=[...]` override for Codex. Your own ~/.claude/settings.json is merged into, not replaced; ~/.codex/config.toml is never touched, but a notify you configured there does not fire in a Veld terminal — Codex's -c overrides that key rather than merging it. Codex tells Veld less than Claude on purpose: it has a richer hooks system too, but using it means an interactive trust prompt or a flag that waives trust for every hook, not just this one, so Veld uses the older notify hook instead, which only fires at the end of a turn — a Codex pane goes straight from launched to finished with no waiting signal in between, and its spinner (Show what is working) stays blank the whole time it is genuinely working. Anything that is not a plain interactive launch — `claude mcp`, `claude -p …`, `codex exec …`, or a settings/config flag of your own — is passed through untouched (`codex resume`/`fork` count as interactive). Takes effect for new terminals."
              >
                <Checkbox
                  size="xs"
