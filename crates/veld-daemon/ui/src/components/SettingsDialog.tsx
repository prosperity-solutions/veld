          ...
          >
            <Stack gap="md">
              ...
              <Row
                ...
                help="Works with Claude Code and Codex CLI. Neither tool's output says whether it is thinking or waiting — measured against Claude Code, which emits a title sequence, hyperlinks and a progress report, and nothing about its state. So Veld puts a wrapper on the terminal's PATH that hands the real binary an extra, throwaway hook configuration: a settings file for Claude, a `-c notify=[...]` override for Codex. Your own ~/.claude/settings.json and ~/.codex/config.toml are never touched, and no .claude/ is written into your project. Codex tells Veld less than Claude — its notify hook only fires at the end of a turn, so a Codex pane goes straight from launched to finished with no waiting signal in between. Anything that is not a plain interactive launch — `claude mcp`, `claude -p …`, `codex exec …`, or a settings/config flag of your own — is passed through untouched. Takes effect for new terminals."
              >
                <Checkbox
                  size="xs"
