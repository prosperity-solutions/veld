...
**Updates come at the pace you pick.** The app checks for new releases in the background and asks before installing one, and *how often it asks* is `desktop.updateFrequency` in *Settings → General* ("How eagerly to offer updates"). The default, **Once it has settled**, waits until a release is a day and a half old — the fix for a bad release lands within hours, so waiting skips the dialog for both — or until a fourth one is queued up behind it, and it never asks more than once a day. **Every release** checks hourly and offers whatever it finds; **Rarely** gives it three days to settle and two days between prompts. Whatever you pick, *Check for Updates…* answers immediately, declining now means later rather than until the next launch, and `veld update` in a terminal is unaffected — this setting is about the app asking you, not about the only way to get a release.
