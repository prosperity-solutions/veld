...
**Images go in as images, and files go in as paths.** `⌘V` with a picture on the clipboard, or an image dropped onto a pane, arrives in a coding agent as an *attachment* — `[Image #1]`, the same thing Claude Code's own `Ctrl+V` produces — not as a path you then have to hope it reads. A pty carries bytes rather than pictures, so no terminal can send an image down the stream; what agents do instead is read the system clipboard themselves when they see `Ctrl+V`, and that is what Veld arranges. Drop **any other file** and its absolute path is typed at the prompt instead, shell-escaped, several files giving several paths — which is how you point an agent at something without going to look up where it lives.

Which of the two you get is decided by what is actually reading the terminal. At a **shell prompt** you always get the path, because a shell can do something with a path (`open`, `cp`) and nothing with a picture. Inside a **running agent** you get the image. Veld can tell the difference because it already knows both halves: the `claude`/`codex`/`pi` wrappers it puts on the terminal's PATH say an agent is there, and the pty itself says whether anything other than the shell is reading. A full-screen program that is not an agent — `vim`, say — gets the path, not a keystroke it would read as something else.
