...
**A pane behaves like the terminal you came from.** `Shift+Enter` inserts a newline instead of submitting, which is what Claude Code and other coding agents read as a line break in their input box. On macOS, `⌘←`/`⌘→` move to the start and end of the line and `⌘⌫` deletes it, the same substitution Ghostty and iTerm2 make (`⌥`+arrows already move by word). **Drop a file onto a pane** and its absolute path is typed at the prompt — several files give you several paths — which is how you point an agent at something without going to look up where it lives. **`⌘V` with a picture on the clipboard** pastes the picture: a terminal cannot carry an image, so Veld writes it to a temp file and types that path, which every coding agent reads as an image. Screenshot, `⌘V`, ask about it.
