...
- **Run history** — a stopped, failed, or crashed run persists as history (last 10 per environment, 7 days) with its logs; `veld runs` lists it, `veld logs --run <id>` and `veld logs --previous` target it, and a crash is distinguishable from a clean stop
- **Setup / teardown** — project-level lifecycle steps that gate startup (check Docker, create networks) and clean up when the run ends — including when it crashes
...
- **Keyboard shortcuts** — navigate worktrees and tabs, split the dock, open and close tabs, toggle focus mode, switch between the IDE and Runs views, and start/stop/restart a run, all without the mouse. Navigation is Tab-shaped: `⌃Tab`/`⌃⇧Tab` steps tabs, and `⌥Tab`/`⌥⇧Tab` steps worktrees on macOS (`Ctrl+Shift+B`/`Ctrl+Shift+N` on Linux and Windows, where `Alt+Tab` belongs to the window switcher). Shift is the reverse in every case, and the chords keep working while a terminal or an embedded browser pane has the keyboard. Every *arrow* chord is deliberately left alone — `⌘←`/`⌘→` is the caret motion Claude Code and Codex expect from a terminal. Tab cycling runs the worktree's *whole* strip, continuing into the windows you have detached panes into and raising one when it gets there. `⌘⇧D` opens to the side: it moves the active tab to the other half of the dock, or — when it is the only tab there — opens a new pane on that side instead, so the chord always gives you a split. In Veld Desktop `⌘T`/`⌘W` open and close a tab and `⌘⇧W` closes the window, as they do in a browser. `⌘⇧J` is the one chord that does not step a list: it jumps straight to wherever a coding agent is waiting for you — or something finished or failed — in **any** project — the keyboard half of the `Next unread` button described below. Like the rest of the navigation family it keeps working while a terminal or an embedded browser pane has the keyboard. The full list, with each platform's own modifier keys, is `⌘/` — or the ⋯ menu → Shortcuts.
...
| `veld open-url <URL\|file>` | Open a web page — or a local file Veld can show — in the Veld window that owns this terminal (a browser pane beside it). Falls back to your system browser outside a Veld terminal, for an origin on the exempt list, for a file kind you have switched off, or when no window is attached — and for a file it falls back to the system *opener*, which for a `.md` is your editor rather than a browser. A Veld terminal points `$BROWSER` at this, so most CLIs reach it without being told to |
...
| `veld share [RUN] [--node <n>]... [--ttl <secs>] [--approve <first\|manual\|auto>] [--web] [--access <password\|link>] [--password <pw>] [--json]` | Share a running env over an encrypted P2P tunnel; prints a join URL (and `veld join` command). `--ttl` overrides the link's lifetime for this share (default from `sharing.peerTtlMinutes` / `sharing.webTtlMinutes`, or the project's `veld.json`). `--web`: publish the `web`-opted services via the configured gateway and print public URLs — password-protected by default (`--access link` to opt config-silent nodes out, `--password` to choose the password) |
...
| `veld settings [PREFIX] [--json]` | Every Veld setting: effective value, whether it was set or is the default, and what it is. `PREFIX` narrows to whole key segments (`veld settings keepAwake`). These are Veld's own machine-wide preferences — the ones the IDE's settings dialog edits — not a project's config |
| `veld settings get <key> [--json]` | One effective value, printed bare so it drops into a shell substitution |
...
| `veld skills [TOPIC] [--json]` | Veld's own agent documentation, carried by the binary. No topic lists the thirteen; a topic prints it. Needs no project, no config and no daemon — and describes the veld that printed it, which is what makes the installed skills able to stay a pointer |
...
Setup steps that fail (non-zero exit) abort startup with the `failureMessage` if provided. Teardown is best-effort — failures are logged but don't block stop, and a step that hangs is killed after 30 seconds so it can't either. Commands support shell env vars, project-level Veld variables (`${veld.name}`, `${veld.project}`, `${veld.root}`, `${veld.run}`, `${veld.worktree}`, `${veld.branch}`, `${veld.username}`) and `${vars.*}`. Node-scoped built-ins (`${veld.port}`, `${veld.url}`, `${veld.node}`) and `${veld.run_id}` are not available — a project step belongs to no node, and a teardown can outlive the run row.
...
Certificates are issued by that CA for a **week** at a time (under an intermediate good for 30 days), and Caddy renews them on its own once a third of that is left. On a machine that already had veld, the *existing* intermediate keeps its old 7-day life until Caddy rotates it, and a leaf is never allowed to outlive its issuer — so for the first few days after this update, certificates are shorter than a week (whatever is left of the old intermediate) rather than immediately seven days. Nothing to do; the watchdog below measures each certificate against its own lifetime, not against a fixed number. Veld does not take that on trust: every minute the helper completes a TLS handshake on the HTTPS port and reads the validity dates off the certificate it is handed. If Caddy is serving one that has expired — or is so close to expiring that its renewal window has long passed — the helper **restarts Caddy**, which is what actually renews it: Caddy will not re-examine a certificate already in its cache, so reloading its configuration (which veld does on every route change) cannot. `veld doctor` asks the same question and prints the answer as `Certificate:`, next to the `CA:` row that only ever said whether the *authority* was trusted.

The reason both exist: a Caddy whose certificate maintenance had stopped once served an expired certificate for a day and a half. Every check veld had stayed green — the admin API answered, the CA was trusted — while Chrome refused every veld URL with `ERR_CERT_DATE_INVALID`.
...
- **The privileged helper** logs beside its own binary on macOS — `/var/db/veld-helper/veld-helper.log`, since that is the root-owned directory it is served from (`<lib dir>/veld-helper.log` on an install still awaiting that move, and on a legacy `/usr/local` one). It runs as root, so the directory is writable whichever it is, and the file is world-readable so you do not need `sudo` to read it. **On Linux there is no such file**: the helper is a systemd service and its output goes to the journal — `journalctl -u veld-helper`. The *unprivileged* helper has no log file on either platform.
...
One thing deliberately lives outside it: **files handed to a terminal pane**. Dropping a file into a pane, or pasting a screenshot, needs a real path on disk for the program in that pane to read, and a path is not something a database row can be. In the desktop app a dropped file's own path is used and nothing is copied; a pasted image — and any drop from a plain browser tab, where the File API withholds the path — is written to `~/.veld/pastes` (`0700`, files `0600` at creation, overridable with `VELD_PASTE_DIR`). Copies are deleted after a day, by the periodic garbage collection and on the next paste, and `veld uninstall` removes the directory with the rest of `~/.veld`.
...
**Working several projects at once has its own column.** Turn it on with **⌘B**, or the stacked-layers button in the top bar (left of the project selector, `ui.showProjectColumn`, **off** by default — most installs have one project and a column of one square answers nothing) and every project gets a square down the left edge, beside the worktree rail. **⌘1…⌘9** go straight to one, **⌘\`** flips back to the one you came from, and squares are **dragged to reorder** — that order is the daemon's, so it is the same in every window, which is what makes ⌘2 mean one project rather than two. A square is the project's initials — worked out across the whole column, so a family sharing a prefix (`SE-azure-cdn`, `SE-azure-identity`) gets `CD` and `ID` rather than two identical squares — greyscale, so it never competes with the worktree markers beside it, and there is no marker to pick. Right-click one for switch / open-in-a-new-window / remove, and the `+` at the bottom imports another.

**Going back to a project puts you back on the checkout you left it on.** Every way of switching — a square, ⌘1…⌘9, ⌘\`, the project menu, ⌘K — reopens that project's last worktree rather than its main checkout, and the memory outlives a restart. Each Veld Desktop window keeps its own places, so two windows on one project don't drag each other around; a window that has never been on that project starts where *any* window last left it, and a project nothing has ever opened starts on the main checkout as before. (Browser tabs have no window identity to key on, so tabs against one daemon share one memory.) What is remembered is where a window actually ended up: if the remembered checkout is already open somewhere else, the usual hunt lands you on a free one instead of pulling it away, and that becomes the place.
...
A pty carries bytes rather than pictures, so no terminal can send an image down the stream — what actually travels is still a path. What makes it an attachment rather than text is that veld sends it as a **paste** rather than as typing, which is the signal agents use to tell "a file I should look at" from "some characters". The same distinction is why a dropped source file stays text: there is no image there to attach. This is the route iTerm2 and cmux take too, and it needs nothing installed and nothing on your clipboard. It does need the program to have turned bracketed paste on, which is how it can tell a paste from typing — every coding agent does, but that is a property of the agent rather than something veld can guarantee.
...
**Closing a terminal pane** is what ends a shell; closing the browser window or quitting the app only detaches, and a session no window has a pane for any more is hung up after 30 minutes by default (Settings → Terminal) — as is one whose daemon never returns, so nothing is left running for a daemon that is gone for good. **A dropped connection does not start that clock.** A sleeping laptop, a `veld update` and a terminal that has given up reconnecting all detach a session while Veld is still open in front of it, and the shell is kept for as long as some window or tab still has the pane — so a build survives a closed lid and reattaches when you come back, rather than being collected half an hour into the nap. The clock starts when the last window holding it goes. If a foreground process is still running in a terminal, closing it asks first — the same "are you sure, that process will be terminated" a real terminal asks — so an accidental X on a tab running a build or an editor doesn't lose it. Opening the same terminal in a second window takes it over rather than mirroring it, so there is only ever one writer. Up to 48 sessions can be live at once; past that, opening a terminal reports the limit and closing a pane frees a slot. Selecting a worktree no longer opens a terminal by itself: its pane offers the project's panes, a plain shell and the run's URLs, so browsing the rail costs nothing and a shell starts when you ask for one.
...
The shims route a single http(s) URL — or a single **local file** Veld can show (see [Opening a local file in a pane](#opening-a-local-file-in-a-pane)) — and hand **anything else** to the real tool untouched, with the original arguments: `open .`, `open -a Safari …`, `open archive.zip`, a directory, a flag, two arguments. Everything that is not one of those two things behaves exactly as it always did, and the fall-through is silent — `open` is a command people run dozens of times a day, and a wrapper that narrated itself would be noise.
...
**And there is one button that takes you there — `Next unread`, at the end of the top bar** (`⌘⇧J` in Veld Desktop). The rail's glyph tells you a worktree has news, but only once that worktree's *project* is the one selected; the project column's dot says something is happening elsewhere without saying what or where. `Next unread` closes that gap: it goes straight to the pane — one a coding agent is waiting in, or one where something finished or failed — switching project, selecting the checkout and bringing the pane to the front, exactly as clicking the notification does — and it does that from either view, IDE or Runs. **Waiting first, then a failure, then a finish**, the same order the glyphs already rank in: a blocked agent is stopped work and a finished one is not, however recently it finished. Within one of those, the **most recent** goes first — the button appearing and the place it sends you are the same event, so it never opens something you dealt with an hour ago in response to something that just happened. The word is coloured by which kind it is, so you can tell before you read the tooltip — which names the worktree and pane you are about to land on, and the project too when it is not the one already on screen. **Arriving reads the event**, which is the whole of the "next, then next" behaviour: there is no cursor to get out of step, and pressing it until it goes away is how you clear a morning's worth of finished agents. **It is only there while something is waiting** — there is no idle state to render, so there is no greyed one either, and it does not follow *hide inapplicable actions* the way its neighbours do: that setting picks between hiding and greying a control that exists either way, and this one has no such control to pick for. *Settings → General → Show the Next unread button* turns it off entirely; `⌘⇧J` keeps working either way, since a chord costs no room in the bar. Pressed from the Runs view it switches to the IDE once the jump lands — a pane is what it takes you to, so it only changes view when there is something to change it for. It is subject to the same honest limit as the paragraph above — a worktree open in another window is reachable, and that window is raised rather than the pane being stolen.
...
**And it does this by itself while you are sharing.** A share is only useful while the machine serving it is up, so starting one arms the keep-awake for you — *Settings → Keep awake*, on by default, and asked **twice** because the honest answer differs by where your power is coming from. On **mains** it holds for up to two hours and covers a shut lid, which costs nothing there: the macOS flag for a closed lid is valid on AC power only, so no privileged helper is involved. On **battery** it holds idle sleep only, for up to thirty minutes — a laptop that goes in a bag mid-share sleeps exactly the way it always did. Each cap is a *ceiling*, not a countdown: shares expire on their own, so the hold normally just ends when the share does. Unplugging or plugging in re-reads the answer and starts that allowance again, which is what stops the battery limit being sidestepped by starting a share on mains and then walking off with the laptop.

**Two deadlines are in play, and the cup now says which one it is counting.** The hold ends at whichever comes first: the cap above, or the share link's own expiry. With the defaults that is the **cap** — a two-hour mains cap under a four-hour peer link means Veld holds the machine awake for two hours and the link keeps working afterwards, for as long as the machine is up anyway. The two answer different questions, which is why they are not the same number: one bounds what Veld does to your hardware unasked, the other bounds how long a colleague has to open a link.

When the *share* is the shorter one — you passed `--ttl`, the project shortened it, or you raised the cap past the link's life — the cup, the sharing panel, `veld share` and `veld doctor` all say so, because a countdown that does not match the number you set in *Keep awake* is otherwise indistinguishable from that setting being ignored. The link's life is a setting of its own rather than a constant buried in the daemon: *Settings → Sharing*, or `veld settings set sharing.peerTtlMinutes 480`. A project can set it for everyone who checks it out (`sharing.peer_ttl_minutes` / `sharing.web_ttl_minutes` in `veld.json`), and `veld share --ttl` still overrides everything for one link.

**An automatic hold never asks for the privileged helper, on either power source** — that is the line that makes it safe to have on by default. `pmset disablesleep` is durable (see below), and there is no button-press behind an automatic hold to justify writing one. So the strongest thing veld will ever do without being asked is stop the machine dozing off; keeping it awake with the lid shut on battery stays something you ask for, one click on the cup. The cup says which of those you have: it names sharing as the reason, carries the time left, and says when the automatic allowance for a share is used up rather than quietly going out. Turning it off there means off — it will not re-arm itself for the share you turned it off during.

**Outside the IDE it still tells you.** `veld share` prints an `Expires:` line saying when the link itself dies (the lifetime actually in force, whichever of `--ttl`, the project's `veld.json` or your setting decided it) and an `Awake:` line beside `Stop:` saying whether the machine will still be up to serve it, and `veld doctor` reports what is holding it and why — which is the one that answers "why won't this Mac sleep" hours later, from a support transcript, with no window open. There is still deliberately **no keep-awake subcommand**; both of those report state rather than changing it.

That lever is a *durable* system setting rather than an assertion, which is the one thing worth understanding about it: set it and walk away and you have a Mac that never sleeps, with nothing running to explain why. So veld never simply sets it. **The helper holds it on a short lease that the daemon must keep renewing** — the daemon renews every 30s against a 90s lease, and the helper's watchdog puts the setting back once that lease runs out — so a blip costs nothing and a daemon that is really gone costs at most the remainder of the window. A daemon that is killed, wedged, updated or uninstalled stops renewing and the Mac sleeps again on its own; a helper killed mid-lease picks the hold back up when it restarts and gives the daemon a short window to resume renewing — if none does, the same watchdog hands the setting back, and that window is bounded across restarts so a crash loop cannot keep renewing it. Every failure path lands on "the machine can sleep", which is the only safe direction for a setting that survives a reboot. If the lease is ever lost while the keep-awake is still on, the menu stops claiming battery coverage instead of quietly leaving you with a promise nobody is keeping. And if you would rather veld never touched that lever at all, *Settings → Keep awake → Cover a shut lid on battery too* turns it off — which is a guarantee rather than a preference: veld then never writes `pmset disablesleep` on this machine on any path, and a shut lid on battery sleeps.
...
**Settings** (`⌘,`, or *Settings* in the ⋯ menu at the end of the top bar) covers which shell terminals open, terminal font, cursor, scrollback, bell volume, the Shift+Enter behaviour, **how a dropped connection auto-reconnects** (how many attempts, how soon the first is, and the backoff between the rest), what Veld notices about finished commands and waiting coding agents, what it shows in the rail, and which of those send you a system notification, whether terminal links open in a pane, whether file paths in the terminal are clickable, whether `open`/`xdg-open` are caught as well, and which origins are exempt, how long detached shells are kept, whether the project column is shown, whether the top bar carries the Next unread button, how worktrees are marked in the rail, where new worktree checkouts are stored, how long trashed worktrees are kept before they are deleted for good, how far back the run history views reach, which time zone log timestamps are shown in, which quick switches a browser pane's toolbar carries, and whether top-bar actions that currently can't fire (restart with nothing running, the machine-vars button for a project that asks for nothing, a URLs button with nothing to open) are hidden rather than shown greyed. They sit in five groups picked from a sidebar — **General** for the settings that belong to no larger surface (markers, the trash horizon, the history horizon, the log time zone, the hide-vs-greyed choice), then **Git** (which mode the New worktree dialog opens on, where a new worktree's branch starts from, and where new checkouts are stored), **Terminal**, **Links** and **Browser panes** — so one group is on screen at a time rather than every setting on one scroll; on a narrow window the sidebar becomes a select. They are stored by the daemon rather than the browser, so Veld Desktop and a browser tab against the same daemon agree, and every window sees a change. There is no Save button — each control writes as you change it.

**Creating a worktree starts with the work.** *New worktree…* opens on two modes — **Start with a prompt** and **Start with a name** — and the first time you open it, that choice is *all* it shows, with a line on what each one does. Whichever you pick becomes your default (`worktree.newMode`, also in Settings → Git); the control stays at the top of the dialog, so switching is one click and re-records. **Start with a prompt** asks which of the project's agents (`ide.panes`) should pick this up and what needs doing — the agents being the panes that hold a conversation, which veld reads from their `resume` command unless the pane says otherwise with `agent` — and on Create the checkout opens with that agent already running on your prompt, typed in for you, with the name, branch and marker chosen for you. The agent you used is remembered per project, so the next create in this repo opens on it. **Start with a name** is the older dialog: name it, choose where it starts from, pick its marker, and nothing is run. **Nothing in either mode is required**, the prompt included: leave it empty and you get the checkout on its pane chooser, exactly as before. `⌘`/`Ctrl`+`Enter` in the prompt creates it without reaching for the button, since plain `Enter` is a newline there. **The prompt is a draft**: closing the dialog by accident — Esc, the scrim, the ⋯ close — keeps what you typed, per project, and so does reloading the page. It is cleared by a create that actually sends it, so typing a prompt and then making a plain checkout instead leaves it waiting for next time; clearing the field forgets it. Everything the dialog used to ask for is still there under *Start with a name*, and the name is now optional — left empty it is taken from the first clause of your prompt (`Fix the login redirect loop`), or numbered (`Workspace 2`) when there is no prompt either, and moved out of the way of a name or branch a sibling already holds rather than reported as a clash. Type a name yourself and it behaves as it always has: the rail name (`checkout-v2`) and the branch are derived from it and both are shown in the dialog before anything is created, because both derivations drop characters a git ref or a hostname cannot carry. Umlauts and accents are transliterated rather than thrown away — `Größe ändern` becomes `groesse-aendern`, `café` becomes `cafe` — since `groesse` is what a German speaker writes when they have to reach ASCII, and `gr-e` is not. The branch follows the name until you type over it, and clearing that field hands it back to the derivation. The rail marker is picked in the same dialog rather than on a second trip through the context menu, and opens on a **random free** colour and glyph — random rather than "the next unused one", which proposed the same marker for every new checkout in a repo and made a week's rail look dealt off the top of the deck. If the name you typed collides with a checkout this repo already has, the dialog says so before the button rather than quietly appending a `-2`.

**A checkout can start from four places.** *Start from* is the first thing the dialog asks, and **a new branch is the default** — that is what almost every create is, and the other three are for the cases it cannot express. *An existing local branch* checks out a branch git already has, named exactly, with the ones already checked out somewhere else marked as such and refused before the button (git allows one checkout per branch, and the refusal used to arrive as git's own message after the fact). *A remote branch* fetches that remote and cuts a local branch tracking it, so the checkout starts at what the remote has now rather than at the last poll; a remote branch whose local counterpart already exists says so, and points at the local-branch option instead. The lists come from the refs already on disk — reading them never fetches — so a branch pushed seconds ago may not be listed yet. **`origin` is the one kept fresh for you**: the repo poll fetches it about once a minute, while the list covers every remote, so a second remote's branches appear once you have fetched it yourself. The checkout is current either way, because creating one fetches the chosen ref's own remote first.

**And the fourth is another worktree — a spin-off.** *Another worktree* cuts the new branch from that checkout's HEAD, so its commits come along including the ones it has never pushed, and *Carry over uncommitted changes* (on by default) reproduces the rest: staged, unstaged and untracked files, with the staging intact rather than flattened into one pile. Ignored files are deliberately left behind — a `node_modules` or a `target` is tens of gigabytes and is not work. **The source is never modified**, not its working tree and not its index, so spinning off a checkout an agent is writing in cannot disturb it — and if something *does* write it while it is being read, the toast says the two moments were mixed rather than pretending otherwise. (One thing genuinely cannot travel: an **uncommitted** submodule pointer move, because `git worktree add` does not populate submodules in a new checkout at all. The index records the same commit the source's does; it is the un-staged move that has nowhere to land.) A checkout in the middle of a merge is refused before anything is created, because a conflict cannot travel without the merge it belongs to. The same thing is one click from a worktree's ⋯ menu → *Spin off…*, which opens this dialog with that checkout already chosen and files the result into the same group.
...
**Which checkouts are holding work that isn't safe yet.** A rail row carries a small glyph when git says this checkout has something you could still lose: a **pencil** for uncommitted changes, a **broken link** for a detached HEAD — not on a branch, so new commits belong to none — and an **arrow up** for commits that have not been pushed. Uncommitted work ranks first, because it is the only one describing work that exists nowhere else. Hover for the detail, including how many commits and in which direction. It renders in the collapsed rail too, because "which of these still needs me" is a question people ask with the rail narrow.

**Those three, and nothing else.** A checkout that is clean and fully pushed shows *nothing* — a glyph means "this one is holding something", so a mark that appeared on every tidy row would cost the column the only thing it is for. (A branch glyph for the pushed state was built and cut for exactly that reason: it says nothing about unsaved work, and on the main checkout, which never leaves that state, it was lit permanently.)

**It shares one slot with [the activity glyph](#the-worktree-inbox-what-happened-while-you-werent-looking), and activity wins.** Not for want of room: while an agent is running, the tree is *expected* to be dirty, so a pencil beside a spinner reports what the spinner already implies. The same holds for the rest of that vocabulary — those states are news that arrived and has not been read, and news outranks a standing property of the checkout. Once you have read it, the git state takes the slot back. **The tooltip is not folded**, so a spinning row still tells you it has three unpushed commits — the glyph collapses, the facts do not.

**Veld never claims a pull request exists, or does not, or was merged.** A pull request is a forge object and no git command can be asked about one. A branch whose upstream has been *deleted* — what a merged-and-tidied pull request leaves behind — renders **nothing**: git cannot tell that apart from a pull request closed without merging and then deleted, and a mark that is confidently wrong about which teaches people to distrust the whole row. When veld can see the upstream is gone it says so in the tooltip, where a sentence is allowed to be probabilistic; it will not draw a glyph on it. **It only sees that once your own git has pruned** — `git fetch --prune`, `git remote prune origin`, or `remote.origin.prune = true` — because veld's background fetch deliberately does not prune your refs for you.

Real pull-request state is an [`ide.extensions`](docs/configuration.md#ideextensions) status badge, which holds a PR number and can say it properly — veld's own `veld.json` ships one that runs `gh` and shows `PR #283 · merged` in the top bar.

The cost is kept off the thing that draws the rail. The push half — ahead, behind, upstream gone — is one `git for-each-ref` per **repository**, which reads refs and never enters a working tree, so it rides the poll and is always current. The uncommitted half needs a `git status` per **checkout**, so it runs behind the poll on a rate limiter and the row shows no glyph at all until it has an answer. Note that a clean checkout shows no glyph either — absence means "nothing to see here", the same as everywhere else in the rail — so on screen "not yet measured" and "tidy" look alike. They are still separate values underneath, which is what stops veld ever *asserting* a checkout is clean on the strength of a reading it did not take. Nothing runs while no window is open.

**Organising the rail.** With a dozen checkouts, alphabetical stops helping. **Groups** are sections you name yourself — `review`, `spikes`, `client-x` — created from the rail's folder button or a worktree's context menu → *Move to group*, and a worktree is dragged between them — or created straight into one, since every group header carries its own **＋**; a checkout created that way lands at the **top** of the section you started it from, which is where the thing you just made belongs. Within a group you can **drag worktrees into any order** you like, and the **groups themselves are dragged by their headers** to reorder the rail — including above **Worktrees**, the section holding everything you have not filed into a group, which moves like any other section rather than being pinned to the top — the same move the group's ⋮ menu offers as *Move group up* / *Move group down*, which is still there for the keyboard. Anything you have not placed by hand stays alphabetical underneath. A whole section can also be moved or cleared **at once**: the ⋮ menu offers *Move all worktrees to…*, which files every checkout in that group into another one — an existing group, a **new** one you name right there, or back out of any group — and *Move all worktrees to trash…*, which bins them all. Both confirm first and list exactly what they will touch; the trash one also says which of those checkouts have uncommitted changes, since binning keeps them but a later permanent delete will not — it holds the button for a moment while that check runs, and keeps reporting what it finds after releasing it, so one checkout git is slow to answer for cannot hold up the rest. The **Worktrees** section carries the same two entries, which is how a repo that never defined a group files its whole rail into one. The main checkout is never binned. A **detached** checkout is no longer an exception to any of this: it stays in the group you filed it into and a batch acts on it like any other row, with its state shown by the glyph on the row rather than by moving it somewhere else. The rail is also **resizable** — drag its right edge, or focus it and use `←`/`→` — and the width is remembered *per window*, because two windows on two monitors want different rails. Dragging cannot narrow the rail into its collapsed mode: collapsing hides the name and the branch, so it stays a deliberate click on the chevron rather than something a drag can do to you by accident.

**And a section you are not working in folds shut.** Click a group header — anywhere along the bar, or the disclosure triangle at its far left — and that section closes: the header stays, its rows go, so a rail with six groups is as short as the one group you are in. A folded header carries what the hidden rows would have said, in the language the rows say it in: how many are behind it, the same warning triangle if a hidden checkout's run has **failed** or is one veld is **recovering**, the same activity glyph if one has terminal output you have not seen, and the row's own accent when the checkout you are *in* is one of them — so folding a section never loses where you are. Those are indicators, not buttons: a section speaks for several checkouts, so folding is the only thing the bar itself does — the ＋ and the ⋮ sitting on it keep their own meanings. **Nothing springs open.** Drop a worktree on a folded section and the drop lands, the count ticks up and the section stays shut; the same for one created from its ＋, which is what the accent on the header is then telling you. This is the rail's *other* collapse, and the two are independent: narrowing the rail (the chevron above) draws no headers at all, so there is nothing left to fold there, and widening it again finds every section exactly as you left it. Folds are remembered **per repository, on this machine** — they survive a reload and a restart, and two windows on the same project agree with each other as you fold — but unlike groups and order, which are the daemon's, they do not follow you to another machine.
...
**Removing a worktree puts it in the trash.** *Move to trash…* marks the checkout and hands you straight back the app — **nothing is deleted**, the directory stays on disk, and the row moves to a **Trash** section at the bottom of the rail, which starts folded — a pile you mean to forget about should not cost you rail height. Unfolded it shows the two you binned most recently and a **+N** for everything else, which opens the rest when you click it: the trash has no upper bound, and without that a long-lived one would push the groups you actually work in off the top of the rail. Restoring a worktree from there is a real undo, not a race. That also makes the operation instant: awaiting `git worktree remove` on a large checkout is what froze the UI before, and now the request does no slow work at all.
...
It also has **browser panes**: a tab in the same dock that renders one of the run's URLs, with a back/forward/reload row and an address bar. Each pane runs in a **colour-coded session** — its own cookie jar, named after an animal (otter, wombat, gecko…) rather than numbered — so you can be logged in as two different users of your own app side by side and tell which is which from the tab's dot. Sessions are added and removed from the pane's session menu — up to eight alongside the default, remembered per worktree — and the same menu clears any session's data, signing out every pane on it. A pane that can't load tells you *which* problem it hit: nothing listening (start the run), a hostname that doesn't resolve, an untrusted certificate (`veld doctor`), an **expired** one (different advice: Caddy renews these itself and veld restarts it when renewal stops, so the pane says to reload in a moment rather than to go installing a CA), a timeout, or a crash. A browser pane with no URL **is** the run's start page — "Where to?", then the places it can go, each row opening here on click with copy and open-externally beside it. **The list is the run's own URLs and the worktree's recently edited files, and the project's `ide.quicklinks` are behind a *Bookmarks* button** in that heading, which opens all of them in a dialog: with four to eight services per run, inline bookmarks pushed the addresses veld is actually serving into the middle of a screen that scrolled. Typing in the address bar still matches them, because a filter that could not see a bookmark would be a filter lying about its scope. The two kinds still say where they came from: the run's URLs carry a live dot and the service's name, a bookmark carries a bookmark glyph and no live dot, because veld started the first kind and only read the second out of a config — the glyph and the group carry that, not dimmed text, since a row that is perfectly clickable must not read as disabled. The pane does **not** take your keyboard when it opens: it is as often opened to click a URL as to type one.

**Right-click inside the page** for the menu a browser has there (Veld Desktop only — in a plain browser tab the pane is an iframe and your browser's own menu already covers it). On a link: *Open Link in New Tab*, which lands beside the pane on the same session, and *Copy Link Address* — or *Copy Email Address* on a `mailto:`, which copies `someone@example.com` and not the whole `mailto:…?subject=` string, because getting an address into a To: field is the entire point of the gesture — unless the page hid something invisible in it (a newline, a right-to-left override, or a few thousand characters), in which case you get *Copy Link Address* instead and the raw `mailto:` string, where the smuggled character is still visibly escaped. On an image: the image itself, or its address — and on a video or an audio element, its address. On marked text: *Copy*. In a text field: undo, redo, cut, copy, paste and select all. On the page itself, where you aimed at nothing in particular: back, forward, reload and select all. Plus *Inspect Element*, which opens the pane's DevTools at what you clicked. Whatever was under the pointer decides the menu, so a menu raised on a link is about that link and carries nothing else — and a page with a context menu of its own (an editor, a spreadsheet) keeps it rather than being overruled.
...
### Opening a local file in a pane

An agent writes you a slide deck, or an HTML report after analysing something. Looking at it used to mean leaving Veld for Chrome. Now it opens in a pane, three ways:

- **`open deck.html` in a Veld terminal.** The same shim that routes URLs routes a lone path to a file Veld can show. Anything else — a directory, a flag, two arguments, a file kind you have switched off — reaches the real `open` untouched and silently.
- **The *Recently edited* rows**, newest first, because the file you want is almost always the one that just appeared. Each row is the file's name over the folder it sits in, with how long ago it was written on the right, and a glyph for its type. The undecided pane and a blank pane's start page each offer **at most three, from the last day, and only when the run has no URLs of its own** — a running dev server is why that pane is open, and a file list under it competes with the thing you came for. Everything else is behind the **Files** button in those headings, which opens the full list with a search field. Typing in the address bar searches every file by path, not just the three on screen. The list refreshes when you come back to the window and every twenty seconds while you are looking at it, so files an agent writes while you watch a terminal appear on their own.
- **`veld open-url <file>`**, to invoke by hand or from a script.

The file is served over **http from an origin of its own** — `files.veld.localhost`, a host that nothing else answers on — rather than handed to the pane as a `file://` URL, and that is the difference between a deck that works and one that renders blank. A `file://` document has an opaque origin, so Chromium refuses its ES module imports and its `fetch` of a sibling `data.json`; over http both work, and relative `<link>`, `<script src>` and images resolve the way the page's author expected. That origin also carries **no** management API, which is the point: the page is HTML something else generated, and on the daemon's own origin its scripts would be same-origin with every route under `/api`.

What may be opened is yours to set (Settings → Browser panes → *Local files*): **web pages and PDFs** are on, **images** and **plain text** are off, and a **pattern list** scopes by location (`reports/*.xml`) or opts one extension in without its whole group (`*.log` with plain text off). A pattern chooses among the kinds Veld can serve; it cannot add one, and it cannot reach into `node_modules`, `target`, `dist`, `build` or `vendor`, which the scan never descends into. The two that are off are off for the same reason: the list's value is that it is short, and a repository holds vastly more committed images and text files than generated documents — turn images on and a logo and a vendored diagram sit above the report that was just written. Secrets are never served whatever you write there: `.git`, `.env*`, `*.pem`, `*.key` and their neighbours are refused first, and only a closed list of extensions is served at all, so an unlisted file is a 404 rather than a download. Reads are confined to the worktree the pane belongs to, by a server-resolved grant rather than a path the page names — a symlink pointing out of the checkout is refused too.

**A file pane reloads itself when the file changes**, which is the loop the feature exists for: the agent rewrites the deck and the pane shows the new one. Veld watches the file's timestamp; nothing is injected into the page, so what you present is what is on disk. What it watches is **whichever file the pane is showing** — not only the one it was opened on. Follow a relative link from one deck to the next and the watch moves with it, a set of decks that link to each other behaves like one document, and a `#slide-3` fragment is still the same file. Following a link needs the desktop app, like history and sessions above: a cross-origin `<iframe>` in a plain browser tab reports no navigation, so there the pane keeps watching the file it was opened on. The pane's toolbar toggles it for that pane, and the default is a setting.

Each browser pane can also **emulate a device**. The presets are **size classes, not model names** — small/medium/large phone, three tablet sizes, a 14″ laptop, a 24″ monitor, a 27″ widescreen — because a list of named handsets is long, out of date within a year, and never answers the only question you were asking, which is *how wide*. Each class carries the metrics a current device of that class actually reports. Beside them is **Responsive**: it starts at whatever the pane can hold and you **drag its edges**, which is how you find the width your layout actually breaks at. The page reflows *while* you drag — the screen grows from its centre, and the size under your pointer reads out in the toolbar. Any device can be dragged, not just that one — a phone dragged narrower keeps its touch events and user agent and becomes a custom size. Plus rotate, a mobile-user-agent toggle, an explicit custom size, touch events and **safe-area insets**, so a swipe gesture, a `@media (hover: none)` rule or a header pinned to `env(safe-area-inset-top)` is testable without leaving the app. The useful half is the *large* sizes: a pane is small and a desktop layout is not, so a 1440-wide viewport is rendered at 1440 and **scaled to fit the pane**, which is a view no browser window can give you without a second monitor. **Page zoom** sits in the same control, with a reset. Both are remembered per pane and survive a reload, a session switch and a worktree switch. **DevTools** opens per pane in its own window (the ⟨⟩ button) — detached, because a docked inspector and an embedded view fight over the same box. **Safe-area insets** come with the handheld presets: an emulated phone reserves the notch and home-indicator gutters a real one does, and the page reads them through `env(safe-area-inset-*)` — so a layout that pins a header or a bottom bar to the safe area can be checked in the pane instead of on a handset. The phone classes reserve 47/59/62 at the top and 34 at the bottom; **held sideways the sensor housing appears on *both* sides at once and the home indicator shortens to 21**, which is what a real handset reports and is not any rotation of the portrait numbers, so each class carries both sets rather than a transform. Rotate, or drag or type a size that crosses square, and the gutters follow the shape — a landscape viewport with a notch at the top is a state no handset can be in. Tablets reserve a home indicator and nothing else, identically either way up. The screen presets reserve nothing at all — a monitor claiming a notch would make every one of them a worse desktop than the pane it replaced. A menu item turns them off and on per pane; on a screen preset it is inert and says why, because there is nothing there to reserve, while a custom or responsive size, having no class to inherit from, gets the phone set. Dragging a preset's edge makes it a custom size, so gutters at a desktop width are one drag away rather than unavailable. `env(safe-area-max-inset-*)` is set to the same numbers, because an emulated viewport has no retracting browser UI and its inset is therefore already its own maximum. **Off means zero rather than undefined**, and that is not a choice: Chromium defines all four variables and reports `0px` for them with no override at all, so a page written as `env(safe-area-inset-top, 12px)` never sees its fallback whether or not this is on. Touch emulation needs Chromium's debugging channel, which something else can hold, so the pane reports whether touch is actually in force rather than assuming — its device menu says so when it isn't. The safe-area insets and the emulated media features ride that same channel and are reported the same way, and it is the insets that need it most: touch that has been taken away shows up the moment you drag, while gutters quietly reading `0px` look exactly like a page with no notch to clear. The mobile user agent is the *string* only: `navigator.userAgentData` and the `Sec-CH-UA` headers still report your desktop, so an app that branches on client hints rather than the UA string keeps serving its desktop bundle. The menu says that too. A device only takes effect once the pane has a page: an empty pane has no view to emulate against, so the device you pick applies to the first thing it opens. In a plain browser the *sizes* work (the frame really is that many CSS pixels wide, so your media queries respond), but the user agent, touch, safe-area insets, device pixel ratio, page zoom and DevTools all need the desktop app — the menu says so rather than pretending otherwise.
...
The two you reach for constantly get **one-click switches in the pane's toolbar**, either side of the device button: a **phone**, and the page's **colour scheme**. Both are shortcuts into the device menu, which keeps every one of these controls — turning a switch off costs reach, never capability, which is why *which switches appear* is a setting (Settings → Browser panes) rather than two more buttons for everyone. It is a standing choice about whether you want the shortcut at all, not a per-pane one: the setting is global, so off is off in every pane and window. The phone switch applies the **Phone** preset (402 × 874), touch, safe-area insets and mobile user agent included, and its edges are draggable from there — so the resizable viewport is one drag away rather than being what the click spends itself on. Its **off is no emulation at all**, so it answers one question ("am I on the phone") rather than restoring a device you picked earlier. The colour scheme **cycles System → Dark → Light → System**, so a light-only layout bug is one click away too, and **System is the absence of an override** rather than a third value — which is also what releases the debugging channel; it reports what the pane *achieved*, showing as paused rather than set while something else holds Chromium's debugger. In a plain browser the phone switch's *size* works and the colour-scheme one is shown inert, saying why in its tooltip and its accessible name.
...
**`sessions` is the other question a pane can answer: resume a session it never started.** `resume` reopens what *this* pane launched, which is nothing at all the first time you open it in a worktree, and nothing for any conversation from before the pane existed. A pane can name a command that lists what its tool already has on disk:

```jsonc
"sessions": { "label": "Resume an earlier Claude session",
              "argv": ["scripts/veld/claude-sessions.sh"] }
// ...or "ask_first": false to keep click-to-launch and put the list on the card
```

**Clicking the pane then asks which session you want**, with *Start fresh* as the first row and what the script found underneath. Pick a row and **the pane's own `resume` command runs**, with `${veld.pane.token}` set to the value you picked. So a script chooses which session a declared command opens and can never contribute a command — which is why the resumed conversation comes back with every flag the pane's label promises. Asking is the default (`ask_first`) and it is the default on purpose: a picker behind a small control is one most people never find, and *"you already have a conversation about this worktree"* is exactly what somebody needs told **before** they start a second one. It only ever asks when there is something to ask about — a script that found nothing opens the pane exactly as it always did — and *Start fresh* is always the first row, so it is one extra click and never a dead end. Set `ask_first: false` and the click launches instead, with the list in the card's other half. Stdout is one row per line: a value, then optionally a tab and the text to show, then optionally a tab and a quieter detail, so a pipeline ending in `basename` is a working picker with no adapter. **Exit 0 with no output means there are none, and nothing is offered** — the answer a fresh clone gives, and not an error; a non-zero exit still opens the dialog and shows the last line of stderr, because a broken script nobody can see is a broken script that stays broken. It needs a `resume` that references the token, so the `codex resume --last` shape above cannot have one and `veld lint` says so rather than opening the same session whatever you chose. This is the one thing in `ide.panes` that runs a command *before you click a pane* — once, while the chooser is on screen, never on a timer and never from the `+` menu — so it is under the same bounds and the same off switch as `ide.extensions`. And it is worth checking whether your tool has a picker already: `claude --resume` with no argument and `codex resume` both open their own, and a plain pane running one of those is less to maintain.
...
`fixed_label` is the third, and the only one whose default says *let the tool decide*. A declared pane's tab adopts whatever terminal title (OSC 0/2) the process inside it sets, exactly as a plain terminal's does — for a coding agent that title names the task it is on, and a rail of four tabs all reading "Claude" is the thing it fixes. Set `"fixed_label": true` on a pane whose `label` is the landmark instead: `Claude (skip permissions)` has to stay legible as *that* pane, and a live task title would take the answer away. The pane's identity never moves either way — `${veld.pane.id}` and `${veld.pane.label}` are what the config says, notifications are named by the pane rather than by its current title, and the tab is back on its `label` after a reload.

**Restart is the same pair of choices, offered without waiting for the tool to exit.** Right-click a pane's tab: a plain terminal offers *Restart this terminal*, and a pane the daemon still holds a token for offers **Restart and resume** — hang up what is running now, then run the pane's `resume` command, so the tool comes back on the same conversation having re-read whatever it loads at startup. That is the point of it: change a coding agent's settings and apply them to the pane you have been working in, without starting over. *Restart from scratch* sits underneath for when starting over is what you meant, and mints a new token like any fresh launch. Both ask first whenever something is actually running — a restart hangs up an agent mid-turn the same way closing the pane would — and *Restart from scratch* asks even on a pane that has already exited, because abandoning a conversation the token still points at is a loss you cannot see on the screen. The card's own *Start fresh* asks the same question for the same reason, so the two surfaces cannot disagree about it. The scrollback stays as history either way, and while the pane comes back it says so across the whole pane rather than in a corner. `⌘K`'s *Restart this terminal* is the same action, with the same question.
...
The same `/ide` UI as a desktop app: a native window with a menu-bar icon, real Chromium browser panes (working history, page titles, isolated cookie jars, and pages that refuse to be framed), device emulation, a right-click menu inside the page, and per-pane DevTools. Everything else works identically in a browser — the app is a shell around the daemon, not a second implementation.

**The menu-bar icon is optional.** It is where the app says what is running with no window on screen — the live runs, *Open Veld Desktop*, *New Window*, the version and *Check for Updates…* — and it is on by default for that reason. Turn it off in *Settings → General* ("Show the menu bar icon"), or with `veld settings set desktop.menuBarIcon false`, and it leaves the menu bar at once; the app itself stays where it was, reachable from its Dock icon, so what you lose is the ambient status rather than the way in. Like every other preference this is the daemon's, not the app's, so a browser tab can set it too — the app is told, and reads it again on its own ten-second tick for a change made while it had no window open.
...
**Updates come at the pace you pick.** The app checks for new releases in the background and asks before installing one, and *how often it asks* is `desktop.updateFrequency` in *Settings → General* ("How eagerly to offer updates"). The default, **Once it has settled**, waits until a release is a day and a half old — the fix for a bad release lands within hours, so waiting skips the dialog for both — or until the app has seen four pile up, and it never asks more than once a day. **Every release** checks hourly and offers whatever it finds; **Rarely** gives it three days to settle and two days between prompts. Whatever you pick, *Check for Updates…* answers immediately and spends none of that budget; once you've answered about a release it stays quiet for a week, rather than coming back at the next launch; and `veld update` in a terminal is unaffected — this setting is about the app asking you, not about the only way to get a release.

**It needs the veld CLI**, which it does not ship. On a machine that has never had veld the app shows the two commands that get you there — the installer and `veld setup unprivileged` — and waits for the daemon to appear. On a machine that *does* have veld it says something different, which matters most right after an update: the app can reopen before the daemon has finished restarting, so for the first minute it simply says it is starting up. Only after that does it offer `veld setup unprivileged` and `veld doctor` — never the installer, which would have you re-run an install over a machine that already has one.
...
**While a share is live, veld keeps this machine from dozing off** — a share nobody's laptop is awake to serve is a dead link, and the machine suspending is not something a share can do anything about. It is on by default, capped, and asked separately for mains and battery; `veld share` prints an `Awake:` line saying which you got. The full rules, including the one thing it will never do without being asked, are under [Management UI](#management-ui) above.
...
This installs two skills: **`veld`** — the general one, covering CLI usage, `veld.json` configuration, IDE customization and the feedback workflow — and **`veld-launch-feedback-loop`**, which parks an agent on the `veld feedback next` loop to work in-browser review comments one at a time.

**Both are pointers, and stay a few kilobytes.** The documentation itself is carried by the veld binary and served by `veld skills`, so an agent reads the docs for *the veld you have installed* rather than for whichever version was current the day you ran `npx skills add`. That also means the skills never need re-adding, and that nothing enters your agent's context until it asks:

```sh
veld skills              # the index — thirteen topics, one line each
veld skills basics       # the command surface and the traps, ~4KB
veld skills config       # the full veld.json reference, when authoring one
```

Project state is on the same terms: `veld presets`, `veld nodes` and `veld status` answer *this* project's questions when they come up, instead of a whole `veld.json` being pasted in up front.

**Already installed the old skill?** Run `npx skills add prosperity-solutions/veld` once more. What you have is the previous 100 KB copy; the re-add is what replaces it with the pointer, and it is the last one you will need.
