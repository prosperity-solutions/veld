...
**Which means the number the cup shows is usually the share's, not the cap's — and it now says so.** A share link's own life (two hours peer, one hour web by default) is shorter than any ceiling somebody would set, so a machine set to "at most four hours" and sharing normally reports *1h 59m left*: correct, and thoroughly confusing until you know which of the two deadlines you are looking at. The menu and the sharing panel now say which deadline it is, and the link's life is a setting of its own rather than a constant buried in the daemon: *Settings → Sharing*, or `veld settings set sharing.peerTtlMinutes 240`. A project can set it for everyone who checks it out (`sharing.peer_ttl_minutes` / `sharing.web_ttl_minutes` in `veld.json`), and `veld share --ttl` still overrides everything for one link.
