...
**Terminals are polite about finishing.** The basics of terminal ergonomics are covered: a `BEL` rings the bell, a process can rename its own tab with the terminal title it sets (OSC 0/2), and the OSC 9 "notify" sequence raises a notification. A notification names the **worktree and pane** so it is navigable across several open checkouts, and clicking it focuses the pane that produced it — whether the worktree lives in this window, another Veld window, or a browser tab (the banner is an OS notification, so a backgrounded window still surfaces it). It is deliberately **stateless** — there is no inbox, nothing is persisted, and it is not a dashboard of what an agent is doing; it is just "something in a terminal finished". A plain terminal (a login shell) always adopts its title and notifies; a config-declared pane keeps its configured `label` unless the repo opts in with `allow_terminal_renaming`, because a fixed label is how you navigate a rail full of agent panes.
