...
**And a file path in the terminal opens in your editor.** `src/api.ts`, or a compiler's `crates/veld-daemon/src/pty.rs:2529:17`, is a link too — clicking it runs whichever editor your project declares for the job ([`ide.extensions`](docs/configuration.md#opening-a-clicked-file) with `accepts: "file"`), at that line. Veld checks the path is really a file in the worktree before running anything, and hands it over as a positional parameter rather than building it into a command string, because the path is whatever an agent or a build tool happened to print. What gets underlined is narrow on purpose — a directory or a known source extension, not `example.com` or `v1.2.3` — and *Settings → Links* turns it off for a project whose output is full of path-shaped prose.
