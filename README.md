...
**Removing a worktree puts it in the trash.** *Move to trash…* marks the checkout and hands you straight back the app — **nothing is deleted**, the directory stays on disk, and the row moves to a **Trash** section at the bottom of the rail, which starts folded — a pile you mean to forget about should not cost you rail height. Restoring it from there is a real undo, not a race. That also makes the operation instant: awaiting `git worktree remove` on a large checkout is what froze the UI before, and now the request does no slow work at all.
