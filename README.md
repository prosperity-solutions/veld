...
- **Presets** — named shortcuts for common selections (`fullstack`, `ui-only`); a preset can compose others with `@name`. Each carries a **stable key** — the number you type at `veld start` never shifts when presets are added — plus an optional `label`, `group`, and `when_to_use`, so a list of thirty is pickable by someone who didn't write it, and by a coding agent reading `veld presets --json`. `default_preset` gives a bare `veld start` a defined answer
