...
The cost is kept off the thing that draws the rail. The push half — ahead, behind, upstream gone — is one `git for-each-ref` per **repository**, which reads refs and never enters a working tree, so it rides the poll and is always current. The uncommitted half needs a `git status` per **checkout**, so it runs behind the poll on a rate limiter and the row shows no glyph at all until it has an answer — a state deliberately distinct from "clean", because an unmounted volume and a tidy worktree must not look the same. Nothing runs while no window is open.
