...
- **Coding agents** — *Settings → Activity → Notice when a coding agent is waiting for you* (`terminal.agentIntegration`). Works in an ordinary terminal **and in a config-declared pane** (`ide.panes`). Claude Code's output does not say whether it is thinking or waiting — measured: its inline TUI emits a title sequence, hyperlinks and a progress report, and nothing about its state. So being told is the only honest way to know. Veld puts a `claude` wrapper on the terminal's `PATH` that hands the real binary an extra, throwaway settings file installing lifecycle hooks; a session stopped at a permission prompt then shows amber beside its worktree. A turn that merely *ended* counts as finished, not as waiting — which is the difference between a badge you trust and one you learn to ignore.

**Your own configuration is never edited.** No `~/.claude/settings.json` merge, no `.claude/` written into your project. The hooks live in a file Veld owns inside its own per-daemon directory, named after the terminal session and rewritten on every launch, and `--settings` *merges* into Claude Code's normal hierarchy rather than replacing it — so everything you configured still applies. The wrapper is also deliberately hard to reach: it passes through untouched for anything that is not a plain interactive launch (`claude mcp …`, `claude update`, `claude -p …`, or a `--settings` of your own), and if it cannot find the real binary, or Veld cannot write the file, it `exec`s `claude` exactly as you typed it.
...
**What it cannot do.** The markers are read by the terminal in your window, so events exist only while a window is *open*: close the tab and its news is gone with it. *Failed* is command-only — Claude Code exposes no clean per-turn failure event, so an agent that errors reads as *waiting for you*, which is what it usually is. And agent integration is Claude Code alone today; Codex's notify hook has a different contract (its event JSON arrives as the final argument rather than on stdin) and is not wired.
