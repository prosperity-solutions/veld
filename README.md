...
**Which checkouts have work in them, without opening any.** Every rail row carries a small glyph for what git says about that checkout, following the life of a branch: a **pencil** for uncommitted changes, an **arrow up** for commits that have not been pushed, a **branch** glyph once everything is on the remote, and a **merge** glyph once the branch's upstream has been deleted — which is what a merged-and-tidied pull request leaves behind, and the usual sign a worktree is finished with. One glyph per row, worst-state-wins, with uncommitted work ranking above the rest because it is the only one describing work that exists nowhere else and the branch glyph ranking last because it is the absence of anything to do; hover for the detail, including how many commits and in which direction. It renders in the collapsed rail too, because "which of these can I throw away" is a question people ask with the rail narrow.

**A branch nobody has pushed shows nothing** — there is no upstream to be in sync with, and that absence is what keeps a never-pushed branch from ever being mistaken for a merged-and-deleted one. **And veld never claims a pull request exists, or does not.** A pull request is a forge object and no git command can be asked about one, so a pushed branch renders the branch glyph whether or not a review is open on it. If you want the real thing on screen, that is what an [`ide.extensions`](docs/configuration.md#ideextensions) status badge is for — veld's own `veld.json` ships one that runs `gh` and shows `PR #283 · merged`.
...
Veld says what git measured and no more. A deleted upstream branch is what it reports, not "merged" — a pull request closed *without* merging and then deleted looks identical to git, and only a code host can tell the two apart. Trashing a worktree keeps its branch either way, so a glyph read the wrong way costs a `git worktree add`, never work.
