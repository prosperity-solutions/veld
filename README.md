...
**And there is one button that takes you there — `Next`, at the end of the top bar** (`⌘⇧J` in Veld Desktop). The rail's glyph tells you a worktree has news, but only once that worktree's *project* is the one selected; the project column's dot says something is happening elsewhere without saying what or where. `Next` closes that gap: it goes straight to the pane a coding agent is waiting in — switching project, selecting the checkout and bringing the pane to the front, exactly as clicking the notification does — and it does that from either view, IDE or Runs. **Waiting first, then a failure, then a finish**, the same order the glyphs already rank in: a blocked agent is stopped work and a finished one is not, however long ago it finished. Within one of those, the oldest goes first, so the agent that has been waiting longest is the one you get to. The word is coloured by which kind it is, so you can tell before you read the tooltip — which names the project, worktree and pane you are about to land on. **Arriving reads the event**, which is the whole of the "next, then next" behaviour: there is no cursor to get out of step, and pressing it until it goes away is how you clear a morning's worth of finished agents. It is only there while something is waiting (or greyed, if you have turned *hide inapplicable actions* off), and it is subject to the same honest limit as the paragraph above — a worktree open in another window is reachable, and that window is raised rather than the pane being stolen.
