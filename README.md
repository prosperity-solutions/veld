...
**Settings** (`⌘,`, or the gear in the top bar) covers terminal font, cursor, scrollback, the Shift+Enter behaviour, how long detached shells are kept, how worktrees are marked in the rail, whether unused worktrees are cleared automatically, and which quick switches a browser pane's toolbar carries. They are stored by the daemon rather than the browser, so Veld Desktop and a browser tab against the same daemon agree, and every window sees a change. There is no Save button — each control writes as you change it.
...
Lanes and order are **yours, not the repository's**. Nothing is written to `veld.json`, so your private rail layout never turns up as a review comment on someone else's pull request. (The project-declared counterpart already exists and is a different thing: `ui.quicklinks` is for links every checkout of the repo should see.)

**Removing a worktree** returns immediately. `git worktree remove` on a large checkout takes long enough to freeze a UI, so *Remove worktree…* records the request and hands you back the app: the row moves to a **Pending removal** group and the work happens in the background while you keep working elsewhere. Any run in that worktree is stopped first. If git refuses — uncommitted changes, a locked worktree, a file open in another process — the worktree **comes back into the rail with the reason on its row**, and says so once in a toast; it is never a silent failure. `Keep it after all` on a pending row calls the removal off — the worker re-checks immediately before deleting anything, so if that succeeds it is honoured rather than racing. Nothing is ever force-removed on your behalf: forcing is a second, explicit click, offered only after git has already told you why it said no.

**Unused worktrees can be cleared automatically** (Settings → Worktrees), and this is **off by default** because it deletes a checkout while you are not looking. Set a number of days and an idle worktree is queued for removal — never the main checkout, never one with a run going, **never one with a terminal open in it**, and never one holding local files git would not miss. That last exclusion matters more than it sounds: `git worktree remove` will happily delete a checkout whose only contents are **gitignored**, which is exactly where `.env` files, credentials and build caches live, so eviction asks a stricter question than git does and skips anything `git status --ignored` reports. A worktree it does queue still goes through the same un-forced removal as a manual one, so anything that changed in the meantime is refused and comes back with the reason.
