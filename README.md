...
Each open terminal has a small holder process of its own, and its socket lives in `~/.veld/pty-<daemon port>/` — `veld doctor` reports how many are running, which is where to look if a terminal did not come back. A socket nobody answers is a holder that is gone; the next daemon start sweeps it.

**Closing a terminal pane** is what ends a shell; closing the browser window or quitting the app only detaches, and a session nobody comes back to is hung up after 30 minutes — as is one whose daemon never returns, so nothing is left running for a daemon that is gone for good. Opening the same terminal in a second window takes it over rather than mirroring it, so there is only ever one writer. Up to 48 sessions can be live at once; past that, opening a terminal reports the limit and closing a pane frees a slot. Selecting a worktree no longer opens a terminal by itself: its pane offers Terminal, Browser and the run's URLs, so browsing the rail costs nothing and a shell starts when you ask for one.
...
> **macOS builds are signed and notarized**, so the download opens without a Gatekeeper detour and the app can update itself. A build you compile yourself (`just desktop-package`) is not: if one of those travels to another machine, Gatekeeper stops it, and the way through is **System Settings → Privacy & Security → *Open Anyway*** or `xattr -dr com.apple.quarantine /Applications/Veld.app`.

Updates are checked in the background and offered, never applied behind your back — applying one restarts the app. The macOS build and the Linux AppImage install the update themselves; a `.deb` install opens the release page instead, because its files belong to your package manager. A macOS app you built yourself does too — it checks its own signature, and macOS only lets an app replace itself when the replacement carries the same one. *Check for Updates…* is in the menu bar icon (macOS) and the application menu.
