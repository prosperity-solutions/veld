...
**Outside the IDE it still tells you.** `veld share` prints an `Awake:` line beside `Stop:` saying whether the machine will still be up to serve the link it just gave you, and `veld doctor` reports what is holding it and why — which is the one that answers "why won't this Mac sleep" hours later, from a support transcript, with no window open. There is still deliberately **no keep-awake subcommand**; both of those report state rather than changing it.
