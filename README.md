...
Note what cannot be intercepted automatically: a tool that calls `open` or `xdg-open` directly instead of reading `$BROWSER`. Prepending a directory to `PATH` does not survive a login shell — macOS's `path_helper` rebuilds `PATH` with the system directories first, and Debian's `/etc/profile` overwrites it outright — and Veld does not rewrite your shell's startup files to win that argument. Veld terminals do export `$VELD_SHIM_DIR`, holding `open`/`xdg-open` wrappers that route a single http(s) URL and hand anything else (`open .`, `open -a Safari …`) straight to the real tool, so one line in your own rc file opts in:

```sh
# ~/.zshrc — route open/xdg-open inside Veld terminals to embedded panes
[ -n "$VELD_SHIM_DIR" ] && PATH="$VELD_SHIM_DIR:$PATH"
```

**Settings** (`⌘,`, or the gear in the top bar) covers terminal font, cursor, scrollback, the Shift+Enter behaviour, whether terminal links open in a pane and which origins are exempt, how long detached shells are kept, how worktrees are marked in the rail, how long trashed worktrees are kept before they are deleted for good, how far back the run history views reach, and which quick switches a browser pane's toolbar carries. They are stored by the daemon rather than the browser, so Veld Desktop and a browser tab against the same daemon agree, and every window sees a change. There is no Save button — each control writes as you change it.
