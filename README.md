...
**Which checkouts have work in them, without opening any.** Every rail row carries a small glyph for what git says about that checkout: a **pencil** for uncommitted changes, an **arrow up** for commits that have not been pushed, and a **branch** glyph once everything is on the remote. One glyph per row, worst-state-wins — uncommitted work ranks first because it is the only one describing work that exists nowhere else, and the branch glyph ranks last because it is the absence of anything to do. Hover for the detail, including how many commits and in which direction. It renders in the collapsed rail too, because "which of these can I throw away" is a question people ask with the rail narrow.
...
**Veld never claims a pull request exists, or does not, or was merged.** A pull request is a forge object and no git command can be asked about one. So a pushed branch renders the branch glyph whether or not a review is open on it, and a branch whose upstream has been *deleted* — what a merged-and-tidied pull request leaves behind — renders **nothing**: git cannot tell that apart from a pull request closed without merging and then deleted, and a mark that is confidently wrong about which teaches people to distrust the whole row. Veld does know the upstream is gone and says so in the tooltip, where a sentence is allowed to be probabilistic; it will not draw a glyph on it. **A branch nobody has pushed shows nothing either**, since there is no upstream to be in sync with.
