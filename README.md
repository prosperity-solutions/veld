...
Veld says what git measured and no more. A deleted upstream branch is what it reports, not "merged" — a pull request closed *without* merging and then deleted looks identical to git, and only a code host can tell the two apart. If you want the real thing, that is exactly what an [`ide.extensions`](docs/configuration.md#ideextensions) status badge is for: veld's own `veld.json` ships one that runs `gh` and shows `PR #283 · merged` in the top bar. Trashing a worktree keeps its branch either way, so a glyph read the wrong way costs a `git worktree add`, never work.
