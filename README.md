...
**Which checkouts are holding work that isn't safe yet.** A rail row carries a small glyph when git says this checkout has something you could still lose: a **pencil** for uncommitted changes, an **arrow up** for commits that have not been pushed. Uncommitted work ranks first, because it is the only one describing work that exists nowhere else. Hover for the detail, including how many commits and in which direction. It renders in the collapsed rail too, because "which of these still needs me" is a question people ask with the rail narrow.
