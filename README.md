...
**The project selector still works the way it did**, for when the column is off: first in the top bar, one click to switch, and a second section listing the same projects under *Open in a new window* (*Open in a new tab* in a plain browser, which is a real second view — a tab takes part in ownership exactly like a window). A dot on the selector itself says one of the projects you are **not** looking at has news; it ignores *running*, since something merely in flight somewhere would light it more or less permanently. Rows say where a project already is — *open in Veld Desktop*, *open in Chrome* — before you click, which is a warning rather than a reason to grey them out: picking one of that project's held checkouts is exactly what brings the other window forward. Going to a *specific* checkout in another project is ⌘K's job: it lists every project's worktrees now, not just the selected project's.

⌘1…⌘9 are **Veld Desktop only** — Chrome and Safari reserve them for their own tabs and a page cannot intercept them. ⌘\` works in both.
