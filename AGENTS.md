...
| `crates/veld-daemon/ui/src/shortcuts/registry.ts` | **Keyboard shortcuts.** The single source of truth for the in-app Shortcuts overview (⋯ menu → "Shortcuts…"). Every row here has a *mechanism* somewhere else, and which one decides what else the diff must touch. **A page-dispatched chord** touches three places: `App.tsx`'s keydown effect, this file, and — easy to miss — `panes/terminalKeys.ts`'s `isAppShortcutChord`, which is what lets a mod+shift chord reach the window listener at all while a terminal pane has focus; skip it and the shortcut silently does nothing in the pane most users spend the most time in. **And it needs a guard: call `ide/dialogGuards.ts`'s `pageChordsBlocked` rather than writing the condition by hand.** A chord that acts while a modal is up acts invisibly, which has shipped three times — `⌘W` closing a terminal behind the Settings modal, `⌃Tab` cycling the strip behind the What's New card, `mod+shift+D` moving a tab between docks behind both. The trap is that the condition has *two* halves: `dialog`, and `promotions.open`, which is a separate modal with its own state and is the one most likely to be up, since the release that adds a chord is the release whose card announces it. `pageChordsBlocked` takes both as required fields so half of it cannot be written. **An Electron menu accelerator** (`⌘N`, `⌘,`, `⌘T`, `⌘W`, `⌘⇧W`) lives in `desktop/src/main.js`'s menu template instead, is handled *before* web contents see the key — which is why it is the right mechanism for anything that must work while a browser pane has focus — and cannot be made conditional, so any guard (a dialog being open, an editable target) has to be written in the renderer that receives it. **A chord forwarded out of a browser pane** additionally needs `desktop/src/browserViews.js`'s `before-input-event`; weigh that one against the guest page, which loses the chord unconditionally — `⌘F` is defensible because the pane substitutes its own find bar, and `⌘⇧`+arrow was tried and reverted because it silently took text selection from every page — which is why the navigation family is Tab-shaped: `⌃Tab` is browser-reserved so a guest page never gets it, and `⌥Tab` only duplicates plain `Tab`'s traversal — a forwarded chord has to leave the page something. Note also that a `Tab` chord **cannot** be a menu accelerator: Chromium's focus manager handles `Tab` before the menu layer, so the accelerator never consumes the key. Arrows are never claimed **by the app** at all: `⌘`+arrow is caret motion, and taking it was a shipped bug. **A fourth mechanism** is a chord the terminal answers *itself*, by substituting bytes — `Shift+Enter`, and the macOS line-editing family (`⌘←`/`⌘→`/`⌘⌫` → `^A`/`^E`/`^U`). Those live in `panes/terminalKeys.ts` alone: no `App.tsx` dispatch and no `isAppShortcutChord` entry, because nothing at the window level wants them. Such a row is tagged `platform: "mac"` where it only exists there, and `ShortcutsDialog` hides a row with no combo on the reader's platform — a row bound on one platform only is legitimate, so `registry.test.ts` requires a combo on *at least one*, not on both. `registry.test.ts` guards this file's own shape and that every page-dispatched mod+shift or mod+alt combo is accepted by `isAppShortcutChord`, but nothing ties a row to the menu template |
...
| `docs/signing-key-rotation.md` | **Org signing key rotation.** The runbook for replacing the ed25519 key the privileged helper verifies against. Touch it whenever `crates/veld-core/src/signing.rs`'s key lists, `veld-sign`'s flags, or `release.yml`'s signing step change — it is the operating manual for a mechanism that runs in production **once**, under pressure, possibly not by its author, and where the failure mode is every privileged install refusing every future release |
...
- **Any user-supplied command executed by a daemon must inherit the user's login-shell `PATH`.** The daemon (launchd), gateway (systemd), and helper run with a bare service `PATH`, so a raw `sh -c` cannot find user-installed CLIs (`op`, `vault`, `pg_isready`, version-manager shims) even though the same command works in the user's terminal. Pass it via `.env("PATH", …)`, resolved by one of the two helpers in `veld_core::user_path`. Anything inside a daemon whose PATH need is directory-independent uses `cached_user_path()` — the desktop directory picker and git plumbing, and `SecretSource::Command` token resolution in `veld-share/src/endpoint.rs` (which `POST /api/shares` reaches while the caller waits); a dedicated `warm_user_path_cache` task keeps that entry warm, deliberately not tied to the health monitor's own scan cadence, which stretches to 300s during a recovery restart. (`spawn_veld` and the health monitor's own liveness probes/restarts have since moved onto the directory-scoped sibling below — this paragraph is still the shape every daemon PATH lookup follows.) `resolve_user_path()` is the uncached primitive, for a one-shot context that resolves once and exits — today only a CLI run's lazy var sources. Resolution spawns a login shell: sub-second normally, up to 10s on a stalled rc file, and the slow rc files belong to exactly the users who need the resolved PATH — which is why a handler must not resolve inline. The cache is one writer (the warm task) publishing into a cell that readers only read, with a single rule doing the work: **a resolution that learned nothing never displaces one that did.** A stall, or a shell that exits zero while answering with nothing better than the process's own `PATH`, leaves the previous value alone — because that value is the bare service PATH, i.e. the bug itself. Resist adding a TTL or a which-resolution-wins rule back: the first version of this cache had both, and each grew its own bug. **The shell resolution spawns is a parameter, and a test never publishes one.** `set_preferred_shell` is process-wide state that *every* concurrently running test in the crate reads through `resolution_shell()`, so a test needing a stub shell hands it to `cached_user_path_for_with_shell`/`resolve_with_fallback` rather than publishing it — the daemon is `set_preferred_shell`'s only caller. Two tests published a stub and made `veld-core`'s suite permanently a few tests red locally while CI stayed green (#310): whichever `values` test ran alongside them resolved `PATH=/old/bin` and could not spawn `cat`. The daemon's own `after_settings_write` republishes on *every* settings write, so a `veld-daemon` test that PATCHes `terminal.shell` is that caller too — patch it back, or don't route through the handler. Same rule for any other process-wide value a test publishes (`HOME`, the `VELD_*` instance variables): take **the crate-wide `test_support::process_state_guard`**, not a module-local mutex — two mutexes over one variable exclude nothing — and restore the previous value on drop. **Two caches, chosen by whether the command belongs to a project.** `cached_user_path()` is the process-wide answer above, right for anything directory-independent (the health monitor's own PATH used to be read once for every project this way; `SecretSource::Command` token resolution; the desktop directory picker). `cached_user_path_for(project_root)` is the per-directory sibling — the one to reach for by default for anything spawning *a project's own declared command*: `spawn_veld` (the top bar's start/stop/restart/action buttons), a liveness probe's `command`/`bash` check, an `ide.extensions` command, a pane's `requires_bin` preflight. A directory-based version-manager hook (`.nvmrc` + `.zshrc`, fnm's directory hook) only fires when the login shell doing the resolving actually starts inside that project's directory; `cached_user_path()` resolves wherever the daemon process itself happens to sit and is shared by every project, so it structurally cannot represent "what would this project's shell see" — however often it is refreshed. This shipped broken once already: a top-bar-started run picked the wrong Node version while the identical command from a terminal pane worked, because the pane's shell resolves its `PATH` by actually sitting in the project directory (see the PTY exception below) and `spawn_veld` did not. Never spawn a config-declared, project-scoped command on a daemon with the *global* cache; use the directory-scoped one unless there genuinely is no project directory in scope (a `SecretSource::Command` reached from a standalone gateway boot, with no single project to key on, is the one place that line is still fuzzy — flagged, not yet resolved). **A login shell is not a substitute for the injection.** `$SHELL -l -c` sources `.zprofile` but not `.zshrc`, which is where version managers (nvm, fnm, rbenv) and `brew shellenv` live on most machines; on Debian `/etc/profile` overwrites `PATH` outright, so a login shell wrapped around an injected PATH *discards* it. That mistake shipped in `spawn_veld` and made every UI- or Desktop-initiated `veld start` resolve node commands against the bare launchd PATH (`sh: npx: command not found`). Spawn the binary directly with the injected PATH; don't reach for a shell to get it. **The PTY holder is the documented exception, and a pane is now part of it.**
...
- **Which client is showing a worktree is the daemon's answer, never a shell's.**
  ...
  only what a daemon cannot do: raise a window (`veld:window:focus-self`; also
  `veld:window:cycle-tab`, which raises whichever window the next tab is in),
  route a cross-window tab drop, and **own the cross-window tab order** — the
  main process is the only place that can see every window, so it keeps each
  one's strip (`veld:window:tabs`) and answers "next tab" from that rather than
  a renderer keeping a registry of the windows it opened. **A browser tab cannot
  be focused** — `window.focus()` outside a user gesture is ignored — so a
  refusal carries the holder's *kind* and the UI says where the worktree is
  instead of promising a raise that will not happen; do not "fix" that by calling
  `focus()` anyway.
