...
- **The node type is `long_running`; `start_server` is a permanent serde alias**
  (exactly as `bash` is for `command`). Write `long_running` in new docs, examples
  and tests; never "fix" the alias out of an existing config, and never add a
  migration for it. The type names the **lifecycle only** — whether a node serves
  anything is a property of its `ports`.
- `command` type steps do NOT get `${veld.port}` — only a `long_running` node
  does, and only when it has a port. `"ports": null` declares a long-running node
  with **no ports at all**: no allocation, no `${veld.port}`, no URL, no route
  (absent `ports` still means one auto `http` port). A port's `protocol` decides
  the rest: `http` mints a hostname (`<node>` for the primary, `<node>-<port>` for
  a secondary) plus a Caddy route and `${veld.urls.<name>}`/`VELD_URL_<NAME>`;
  `tcp` is allocated and exported (`${veld.ports.<name>}`, `VELD_PORT_<NAME>`) and
  never routed. The default is `http` for the primary port and `tcp` for every
  other — that asymmetry is what keeps the feature invisible to existing configs,
  so don't "simplify" it to one default.
